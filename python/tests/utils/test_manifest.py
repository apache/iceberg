...
def test_read_manifest_entry(generated_manifest_entry_file: str) -> None:
    ...
    manifest_entries = list(read_manifest_entry(input_file, format_version=1))
    manifest_entry = manifest_entries[0]

    assert manifest_entry.status == ManifestEntryStatus.ADDED
    assert manifest_entry.snapshot_id == 8744736658442914487
    assert manifest_entry.sequence_number == None
    assert isinstance(manifest_entry.data_file, DataFile)

    data_file = manifest_entry.data_file

    assert data_file.content == DataFileContent.DATA
    assert data_file.file_path == "/home/iceberg/warehouse/nyc/taxis_partitioned/data/VendorID=null/00000-633-d8a4223e-dc97-45a1-86e1-adaba6e8abd7-00001.parquet"
    ...
    assert data_file.distinct_counts == None
    ...
    assert data_file.distinct_counts == None
    assert data_file.key_metadata == None
    assert data_file.split_offsets == [4]
    assert data_file.equality_ids == None
...
def test_read_manifest_list(generated_manifest_file_file: str) -> None:
    ...
    manifest_list = list(read_manifest_list(input_file, format_version=1))[0]
