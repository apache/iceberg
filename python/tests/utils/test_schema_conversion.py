...
def test_avro_to_iceberg(avro_schema_manifest_file: Dict[str, Any]) -> None:
...
def test_iceberg_to_avro_manifest_list(avro_schema_manifest_file: Dict[str, Any]) -> None:
    """Round trip the manifest list"""
    iceberg_schema = AvroSchemaConversion().avro_to_iceberg(avro_schema_manifest_file)
    avro_result = AvroSchemaConversion().iceberg_to_avro(iceberg_schema, schema_name="manifest_file")
    assert avro_schema_manifest_file == avro_result
