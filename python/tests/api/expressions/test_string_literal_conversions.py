...
class TestStringLiteralConversions(unittest.TestCase):

    def test_string_to_string_literal(self):
        string = Literal.of("abc")
        self.assertEqual(string, string.to(StringType.get()))

    def test_string_to_date_literal(self):
        ...
        self.assertEqual(avro_val, date.value)

    def test_string_to_time_literal(self):
        ...
        self.assertEqual(avro_val, time_lit.value)

    def test_string_to_timestamp_literal(self):
        ...
        avro_val = avro_conversion["long-timestamp-micros"](dateutil.parser.parse("2017-08-18T14:21:01.919+00:00"),
                                                            None)
        self.assertEqual(avro_val, timestamp.value)
    ...
    @raises(RuntimeError)
    def test_timestamp_with_zone_without_zone_in_literal(self):
        timestamp_str = Literal.of("2017-08-18T14:21:01.919")
        timestamp_str.to(TimestampType.with_timezone())

    @raises(RuntimeError)
    def test_timestamp_without_zone_with_zone_in_literal(self):
        timestamp_str = Literal.of("2017-08-18T14:21:01.919+07:00")
        timestamp_str.to(TimestampType.without_timezone())

    def test_string_to_uuid_literal(self):
        expected = uuid.uuid4()
        uuid_str = Literal.of(str(expected))
        uuid_lit = uuid_str.to(UUIDType.get())

        self.assertEqual(expected, uuid_lit.value)

    def test_string_to_decimal_literal(self):
        decimal_str = Literal.of("34.560")
        decimal_lit = decimal_str.to(DecimalType.of(9, 3))

        self.assertEqual(3, abs(decimal_lit.value.as_tuple().exponent))
        self.assertEqual(Decimal("34.560").as_tuple(), decimal_lit.value.as_tuple())

        self.assertIsNone(decimal_str.to(DecimalType.of(9, 2)))
        self.assertIsNone(decimal_str.to(DecimalType.of(9, 4)))
