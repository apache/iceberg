...
from iceberg.avro.reader import AvroStruct
...
test_files = [
    ("deflate", data_dir + "manifest_deflate.avro", b"B\x9e\xb0\xc3\x04,vG\xd1O\x8e\xf5\x9c\xdd\x17\x8d"),
    ("null", data_dir + "manifest_null.avro", b"_\x82\t\xc4\x94l\x87W\x1e\x03\x170\xc0\xdc2\xe6"),
    ("snappy", data_dir + "manifest_snappy.avro", b'\x81\xc0Q\xf4\xe0"\xe1\xe3c\xcd\xde/7\xc6\xab\x03'),
]


@pytest.mark.parametrize("compression,test_file,sync_marker", test_files)
def test_read_header(compression: str, test_file: str, sync_marker: bytes):
    with AvroFile(LocalInputFile(test_file)) as reader:
    ...
    assert header.sync == sync_marker


@pytest.mark.parametrize("compression,test_file,sync_marker", test_files)
def test_reader_data(compression: str, test_file: str, sync_marker: bytes):
    with AvroFile(LocalInputFile(test_file)) as reader:
