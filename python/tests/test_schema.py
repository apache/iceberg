...
#         (
#                 schema.Schema(
#                     NestedField(1, "foo", StringType()), NestedField(2, "bar", IntegerType(), required=False),
#                     schema_id=1
#                 ),
...
def test_schema_index_by_id_visitor(table_schema_nested):
    ...
    assert index == {
        ...
        8: NestedField(
            field_id=8,
            name="value",
            field_type=MapType(key_id=9, key=StringType(), value_id=10, value=IntegerType(), value_required=True),
            required=True,
        ),
