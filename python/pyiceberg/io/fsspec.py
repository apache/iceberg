...
class FsspecFileIO(FileIO):
    ...
    def __init__(self, client_kwargs: Optional[dict] = None, **fs_kwargs):
    ...
    def _resolve_fs_type(self, scheme: str):
        ...
        if scheme in ("s3", "s3n", "s3a"):
            ...
            if self._client_kwargs:
                self._fs = S3FileSystem(client_kwargs=self._client_kwargs, **self._fs_kwargs)
                return self._fs
            else:
                self._fs = S3FileSystem(**self._fs_kwargs)
                return self._fs
        else:
