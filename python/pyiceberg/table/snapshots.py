...
from pyiceberg.manifest import ManifestFile, read_manifest_list, DataFile
...
class Snapshot(IcebergBaseModel):
    ...
    def added_data_files(self, io: FileIO) -> List[DataFile]:
...
def current_ancestors(table: "Table") -> Iterable[Snapshot]:
    ...
    """
    return ancestors_of(table.current_snapshot(), table.snapshot_by_id)
...
def current_ancestor_ids(table: "Table") -> Iterable[int]:
    ...
    """
    return ancestor_ids(table.current_snapshot(), table.snapshot_by_id)
...
def oldest_ancestor_after(table: "Table", timestamp_ms: int) -> Snapshot:
    ...
    """
    last_snapshot = table.current_snapshot()
...
def new_files(
    ...
    if last_snapshot.snapshot_id != base_snapshot_id:
        raise ValueError(
            f"Cannot determine history between read snapshot {base_snapshot_id} and the last known ancestor {last_snapshot.snapshot_id}"
        )
