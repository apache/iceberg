...
public abstract class BaseParquetReaders<T> {
  ...
  protected abstract ParquetValueReaders.PrimitiveReader<?> dateReader(ColumnDescriptor desc);

  protected abstract ParquetValueReaders.PrimitiveReader<?> timeReader(
      ColumnDescriptor desc, TimeUnit unit);

  protected abstract ParquetValueReaders.PrimitiveReader<?> timestampReader(
      ColumnDescriptor desc, TimeUnit unit, boolean isAdjustedToUTC);

  protected abstract ParquetValueReaders.PrimitiveReader<?> uuidReader(ColumnDescriptor desc);
  ...
  private class LogicalTypeAnnotationParquetValueReaderVisitor
    ...
    public Optional<ParquetValueReader<?>> visit(
      ...
      return Optional.of(timeReader(desc, unit));
    }
    ...
    public Optional<ParquetValueReader<?>> visit(
      ...
      return Optional.of(
          timestampReader(desc, unit, ((Types.TimestampType) expected).shouldAdjustToUTC()));
    }
    ...
    public Optional<ParquetValueReader<?>> visit(
      ...
      return Optional.ofNullable(uuidReader(desc));
    }
  }
