...
public class TestColumnIndexFilter {
  ...
  private static class CIBuilder {
    ...
    CIBuilder addPage(long nullCount, int min, int max) {
      ...
      nullCounts.add(nullCount);
      minValues.add(
          ByteBuffer.allocate(Integer.BYTES).order(ByteOrder.LITTLE_ENDIAN).putInt(0, min));
      maxValues.add(
          ByteBuffer.allocate(Integer.BYTES).order(ByteOrder.LITTLE_ENDIAN).putInt(0, max));
      return this;
    }
    ...
    CIBuilder addPage(long nullCount, double min, double max) {
      ...
      nullCounts.add(nullCount);
      minValues.add(
          ByteBuffer.allocate(Double.BYTES).order(ByteOrder.LITTLE_ENDIAN).putDouble(0, min));
      maxValues.add(
          ByteBuffer.allocate(Double.BYTES).order(ByteOrder.LITTLE_ENDIAN).putDouble(0, max));
      return this;
    }
    ...
    ColumnIndex build() {
      return invalid
          ? null
          : ColumnIndexBuilder.build(type, order, nullPages, nullCounts, minValues, maxValues);
    }
  }
  ...
  private static final MessageType FILE_SCHEMA =
      org.apache.parquet.schema.Types.buildMessage()
          ...
          .addField(
              optional(BINARY).id(2).as(LogicalTypeAnnotation.stringType()).id(2).named(STR_COL))
          ...
          .addField(
              optional(INT32)
                  ...
                  .named(INT_DECIMAL_7_2))
          .named("table");

  private static final ColumnIndex INT_COL_CI =
      new CIBuilder(optional(INT32).named(INT_COL), ASCENDING)
          .addPage(0, 1, 1)
          .addPage(1, 2, 6)
          .addPage(0, 7, 7)
          .addPage(1, 7, 10)
          ...
          .addPage(0, 24, 26)
          .build();
  private static final OffsetIndex INT_COL_OI =
      new OIBuilder()
          .addPage(1)
          .addPage(6)
          .addPage(2)
  ...
  private static final ColumnIndex STR_COL_CI =
      new CIBuilder(optional(BINARY).as(stringType()).named(STR_COL), DESCENDING)
          ...
          .addPage(0, "Alfa", "Alfa")
          .build();
  private static final OffsetIndex STR_COL_OI =
      new OIBuilder()
          .addPage(1)
          .addPage(3)
          .addPage(4)
          ...
          .addPage(1)
          .build();
  private static final ColumnIndex NO_NANS_CI =
      new CIBuilder(optional(DOUBLE).named(NO_NANS), UNORDERED)
          .addPage(0, 2.03, 2.03)
          .addPage(0, 0.56, 8.71)
          .addPage(2, 3.14, 3.50)
          .addPage(0, 2.71, 9.99)
          ...
          .addNullPage(4)
          .build();
  private static final OffsetIndex NO_NANS_OI =
      new OIBuilder()
          .addPage(1)
          .addPage(5)
          .addPage(4)
  ...
  private static final OffsetIndex NO_CI_OI =
      new OIBuilder()
          .addPage(1)
          .addPage(3)
          .addPage(2)
          ...
          .addPage(2)
          .build();
  private static final ColumnIndex ALL_NULLS_CI =
      new CIBuilder(optional(INT64).named(ALL_NULLS), ASCENDING)
          .addNullPage(1)
          .addNullPage(29)
          .build();
  private static final OffsetIndex ALL_NULLS_OI = new OIBuilder().addPage(1).addPage(29).build();
  private static final ColumnIndex INT_DECIMAL_7_2_CI =
      new CIBuilder(optional(INT32).named(INT_DECIMAL_7_2), UNORDERED)
          .addPage(0, 99, 99)
          .addPage(0, 100, 100)
          .addPage(0, 101, 101)
          .addPage(0, 98, 98)
          ...
          .addNullPage(2)
          .build();
  private static final OffsetIndex INT_DECIMAL_7_2_OI =
      new OIBuilder()
          .addPage(1)
          .addPage(3)
          .addPage(2)
          ...
          .addPage(2)
          .build();
  private static final ColumnIndexStore STORE =
      new ColumnIndexStore() {
        @Override
  ...
  private void assertRowRangesEquals(RowRanges expected, RowRanges actual) {
    if (!rowRangesEquals(expected, actual)) {
      throw new AssertionError(
          String.format("RowRanges are not equal, expected: %s, actual: %s", expected, actual));
    }
  }
  ...
  private RowRanges calculateRowRanges(
      Schema schema,
      MessageType messageType,
      Expression expr,
      boolean caseSensitive,
      ColumnIndexStore store,
      long rowCount) {
  ...
  public void testMissingColumn() {
    Assert.assertThrows(
        ...
        () -> calculateRowRanges(equal("missing", 0)));
  }
  ...
  @Test
  public void testNot() {
    // ColumnIndexEvalVisitor does not support evaluating NOT expression, but NOT should be
    // rewritten
    RowRanges expected;
  ...
  public void testStringStartsWith() {
    ...
    assertRowRangesEquals(
        expected,
        calculateRowRanges(Expressions.or(startsWith(STR_COL, "Q"), startsWith(STR_COL, "G"))));
  ...
  public void testIntTypePromotion() {
    ...
    RowRanges actual =
        calculateRowRanges(
            promotedLong, FILE_SCHEMA, equal(INT_COL, 0), true, STORE, TOTAL_ROW_COUNT);
    assertRowRangesEquals(expected, actual);

    expected = selectRowRanges(INT_COL, 2, 3);
    actual =
        calculateRowRanges(
            promotedLong, FILE_SCHEMA, equal(INT_COL, 7), true, STORE, TOTAL_ROW_COUNT);
    assertRowRangesEquals(expected, actual);
  }
  ...
  public void testMissingOffsetIndex() {
    ...
    PrimitiveType missingOI =
        org.apache.parquet.schema.Types.primitive(INT32, Type.Repetition.REQUIRED)
            .id(1)
            .named("missing_oi");
    ...
    expected = ALL_ROWS;
    RowRanges actual =
        calculateRowRanges(SCHEMA, messageType, equal(INT_COL, 1), true, STORE, TOTAL_ROW_COUNT);
    assertRowRangesEquals(expected, actual);
  }
  ...
  public void testIntBackedDecimal() {
    ...
    expr =
        or(
            lessThan(INT_DECIMAL_7_2, new BigDecimal("1.00")),
  ...
  public void testDecimalTypePromotion() {
    ...
    Schema promotedDecimal =
        new Schema(Types.NestedField.optional(6, INT_DECIMAL_7_2, Types.DecimalType.of(38, 10)));
  ...
  public void testBinaryBackedDecimal() {
    ...
    Schema schema =
        new Schema(Types.NestedField.optional(1, binaryDecimal, Types.DecimalType.of(38, 10)));
