...
public class TestCharSeqComparator {
  ...
  public void testCharOrderBeforeLength() {
    ...
    // Sanity check that String.compareTo gives the same result
    assertThat(s1.compareTo(s2) > 0).as("First difference takes precedence over length").isTrue();
    assertThat(s2.compareTo(s1) < 0).as("First difference takes precedence over length").isTrue();
