...
public class ResidualEvaluator implements Serializable {
  ...
  private class ResidualVisitor extends BoundExpressionVisitor<Expression> {
    ...
    public <T> Expression endsWith(BoundReference<T> ref, Literal<T> lit) {
      return ((String) ref.eval(struct)).endsWith((String) lit.value()) ? alwaysTrue() : alwaysFalse();
    }

    @Override
    public <T> Expression notEndsWith(BoundReference<T> ref, Literal<T> lit) {
      return ((String) ref.eval(struct)).endsWith((String) lit.value()) ? alwaysFalse() : alwaysTrue();
    }
