...
enum UUIDv7Timestamps implements Transform<UUID, Integer> {
  UUID_TO_YEAR(ChronoUnit.YEARS, "year"),
  UUID_TO_MONTH(ChronoUnit.MONTHS, "month"),
  UUID_TO_DAY(ChronoUnit.DAYS, "day"),
  UUID_TO_HOUR(ChronoUnit.HOURS, "hour");

  private static final int VERSION_AND_RAND_A_BITS = 16;

  @Immutable
  static class Apply implements SerializableFunction<UUID, Integer> {
  ...
  UUIDv7Timestamps(ChronoUnit granularity, String name) {
    this.name = name;
    this.granularity = granularity;
  ...
  @Override
  public SerializableFunction<UUID, Integer> bind(Type type) {
  ...
  @Override
  public UnboundPredicate<Integer> project(String fieldName, BoundPredicate<UUID> pred) {
    ...
    if (pred.isUnaryPredicate()) {
      ...
      return ProjectionUtil.truncateArray(fieldName, pred.asLiteralPredicate(), apply);
    } else if (pred.isSetPredicate() && pred.op() == Expression.Operation.IN) {
      return ProjectionUtil.transformSet(fieldName, pred.asSetPredicate(), apply);
    }
    return null;
  }

  @Override
  public UnboundPredicate<Integer> projectStrict(String fieldName, BoundPredicate<UUID> pred) {
    if (pred.term() instanceof BoundTransform) {
      return ProjectionUtil.projectTransformPredicate(this, fieldName, pred);
    }
    if (pred.isUnaryPredicate()) {
      ...
      return ProjectionUtil.truncateArrayStrict(fieldName, pred.asLiteralPredicate(), apply);
