...
enum Timestampns implements Transform<Long, Integer> {
  ...
  static class Apply implements SerializableFunction<Long, Integer> {
    ...
    @Override
    public Integer apply(Long timestampNanos) {
      ...
      switch (granularity) {
        case YEARS:
          return DateTimeUtil.nanosToYears(timestampNanos);
        case MONTHS:
          return DateTimeUtil.nanosToMonths(timestampNanos);
        case DAYS:
          return DateTimeUtil.nanosToDays(timestampNanos);
        case HOURS:
          return DateTimeUtil.nanosToHours(timestampNanos);
        default:
  ...
  Timestampns(ChronoUnit granularity, String name) {
    this.granularity = granularity;
    this.name = name;
  ...
  @Override
  public Integer apply(Long timestampNanos) {
    return apply.apply(timestampNanos);
  }
  ...
  public boolean canTransform(Type type) {
    return type.typeId() == Type.TypeID.TIMESTAMPNS;
  }
