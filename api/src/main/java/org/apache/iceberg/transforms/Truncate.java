...
class Truncate<T> implements Transform<T, T>, Function<T, T> {
   ...
   * never diverge.
   */
  static Transforms.StringTruncateRewrite lengthRewrite(CharSequence literal, int width) {
    // TruncateString keeps the first `width` code points (UnicodeUtil.truncateString), so the
    // literal must be measured in code points too. Using CharSequence.length() would misclassify
    // any literal containing a surrogate pair.
    String value = literal.toString();
    int literalLength = value.codePointCount(0, value.length());
  ...
  private static class TruncateString extends Truncate<CharSequence>
    ...
    public UnboundPredicate<CharSequence> project(
      ...
      if (predicate.isUnaryPredicate()) {
        ...
        switch (pred.op()) {
          case STARTS_WITH:
            switch (Truncate.lengthRewrite(pred.literal().value(), width())) {
              case EXACT:
          ...
          case NOT_EQ:
            ...
            if (Truncate.lengthRewrite(pred.literal().value(), width())
