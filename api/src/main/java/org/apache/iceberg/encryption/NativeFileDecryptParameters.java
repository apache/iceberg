...
/**
 * Barebone decryption parameters, one object per content file.
 * Carries the file and column encryption keys (and optional AAD prefix).
 */
public class NativeFileDecryptParameters {
  private ByteBuffer fileAadPrefix;
  private Map<String, ByteBuffer> columnKeys;
  private ByteBuffer fileKey;

  private NativeFileDecryptParameters(Map<String, ByteBuffer> columnKeys, ByteBuffer fileKey,
  ...
  public static class Builder {
    ...
    /**
     * Set column keys.
     * Key retrieval can require authorization checks and involve interaction with a KMS. In this case, supply
     * keys only for projected columns.
    ...
    /**
     * Set optional AAD prefix.
     * @param aadPrefix Will be used by Parquet for cryptographic verification of file identity
     */
    ...
    public NativeFileDecryptParameters build() {
      return new NativeFileDecryptParameters(columnKeys, fileKey, fileAadPrefix);
    }
  }
