...
3. Parameters cannot have default values; all arguments must be explicitly provided.
4. Variadic (vararg) parameters are not supported. Each definition must declare a fixed number of parameters.
5. Each parameter input MUST be assignable to its declared Iceberg type. For complex types, the value’s
...
6. The `return-type` is immutable. To change it, users must create a new definition and remove the old one.
7. The function MUST return a value assignable to the declared `return-type`, meaning the returned value’s type and
