...
Default value can be changed after being set the first time, but the new default value will only have effect on the new rows to be inserted by the user in the future, it will not affect how the existing rows are read. 
