...
When removing a data file, writers must also remove any deletion vector that applies to that data file from delete manifests. Puffin files that contain removed deletion vectors can be left intact and will be cleaned up during snapshot expiry once all deletion vectors in those files become unreachable.
