...
* `_row_id` a unique long identifier for every row within the table. The value is assigned via inheritance when a row is first added to the table and the existing value should be explicitly written when the row is copied into a new file.
* `_last_updated_sequence_number` the sequence number of the commit that last updated a row. The value is inherited when a row is first added or modified and the existing value must be explicitly written when the row is written to a different data file but not modified.
...
The semantics of whether an operation affecting existing rows is modeled as deleting all modified rows and adding new rows, or upsert with preserved row ids is left to the implementing engine.
