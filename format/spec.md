...
5. The `blob_offset` and `blob_size_in_bytes` fields are used to reference a specific blob in a Puffin file for direct access to a deletion vector. The values must exactly match the `offset` and `length` stored in the Puffin footer for the deletion vector blob.
...
* Deletion vectors are added in v3, stored using the Puffin `delete-vector-v1` blob type
* Manifests are updated to track deletion vectors:
    ...
    * `blob_offset` was added and must match the deletion vector blob's offset in a Puffin file
    * `blob_size_in_bytes` was added and must match the deletion vector blob's length in a Puffin file
