...
The calculation is: `data_space_field_id_start + num_stats_per_column * table_field_id`.
If the table field ID is a [reserved field ID](#reserved-field-ids) then a slightly adjusted calculation is used: `metadata_space_field_id_start + num_stats_per_column * (num_reserved_field_ids - (Integer.MAX_VALUE - metadata_field_id))`.

The individual variables are defined as following:

* `data_space_field_id_start = 10_000`
* `metadata_space_field_id_start = 2_147_000_000`
* `num_stats_per_column = 200`, thus supporting a range of **200** potential stats fields for a single table field
...
This would be `(stats_field_id - data_space_field_id_start) / num_stats_per_column` for normal table field IDs and `stats_field_id - num_reserved_field_ids + (Integer.MAX_VALUE - stats_field_id) + (stats_field_id - metadata_space_field_id_start) / num_stats_per_column` for [reserved field IDs](#reserved-field-ids).
