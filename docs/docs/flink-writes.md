...
- **Iceberg version upgrade**: The dynamic sink committer expects a single commit request per (table, branch, checkpoint). Jobs upgrading directly from Iceberg 1.10 or earlier carry state with multiple commit requests per checkpoint, which this version rejects. To migrate the state, upgrade the job to Iceberg 1.11 first and take a savepoint — the 1.11 committer merges the state into a single commit request per checkpoint — then upgrade to this version. Alternatively, stop the job with `stop-with-savepoint --drain` and restart without state.
