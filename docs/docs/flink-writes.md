...
* It can result in unbalanced traffic distribution if cardinality of the partition key or
...
* Table is partitioned by country code, where some countries (like US) have
...
about the new key.
* For high cardinality scenario (like millions or billions),
