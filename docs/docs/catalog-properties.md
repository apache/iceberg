...
| Property                | Default           | Description                                                                                                                                                           |
|-------------------------|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `token`                 | null              | A Bearer token to interact with the server. Either `token`, `token-path`, or `credential` is required.                                                                |
| `token-path`            | null              | Path to a file containing a Bearer token, as an alternative to `token`. The file is periodically re-read ahead of the token's expiration, so a token rotated in place (for example, a Kubernetes projected service account token) is picked up without restarting the process. Mutually exclusive with `token`.  |
| `token-path-refresh-buffer-ms` | 300000 (5 min) | Time in milliseconds before a `token-path`-sourced token's expiration at which the file is re-read to pick up a rotated token.                                |
| `credential`            | null              | Credential string in the form of `client_id:client_secret` to exchange for a token in the OAuth2 client credentials flow. Either `token`, `token-path`, or `credential` is required. |
