...
| snapshot-expiration-reset-strategy  | connector.snapshot-expiration-reset-strategy    | N/A                          | DEFAULT                          | Strategy for Incremental Streaming Mode to Handle Snapshot Expiration. LATEST: Reset the last snapshot to the latest snapshot.  EARLIEST: Reset the last snapshot to the oldest available snapshot. DEFAULT: The job will fail and restart (if a restart strategy has been configured). This strategy applies exclusively to the FIP27 Source.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 
