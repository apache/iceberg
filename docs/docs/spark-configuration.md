...
| spark.sql.iceberg.executor-cache.expiration-policy    | EXPIRE_AFTER_ACCESS                                         | Policy for expiring executor cache entries: `EXPIRE_AFTER_ACCESS` (default, expires after last access) or `EXPIRE_AFTER_WRITE` (expires after write, regardless of access). |             | 10                                                             | Timeout in minutes for executor cache entries                                                                                   |
