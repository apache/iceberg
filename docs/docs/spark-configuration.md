...
| streaming-starting-offsets | `earliest` | Controls where the stream begins when there is no existing checkpoint. Accepted values: `earliest` (default, start from the oldest snapshot), `latest` (skip all existing data; only process new snapshots), `earliest-with-snapshot` (read all files in the oldest snapshot then switch to incremental), `latest-with-snapshot` (read all files in the current snapshot then switch to incremental). Ignored when a checkpoint already exists. |
