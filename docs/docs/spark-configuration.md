...
| spark.sql.iceberg.view.schema-binding-mode             | BINDING                                                        | Coercion applied to view columns: `BINDING` (widening only), `COMPENSATION` (ANSI cast), `TYPE_EVOLUTION` (no cast)             |
