...
`key_metadata` field. For Avro data tables, the data file length is also added to the `key_metadata`.
...
in the `key_metadata` field. The manifest list file is encrypted by the AES GCM Stream encryption mechanism,
