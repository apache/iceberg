...
| write.metadata.metrics.max-inferred-column-defaults  | 100                         | Defines the maximum number of columns for which metrics are collected. Top level primitive columns are counted first before nested columns                                                        |
