...
| write.metadata.metrics.max-inferred-column-defaults  | 100                         | Defines the maximum number of columns for which metrics are collected. Columns are included breadth first: top level fields first; then elements of nested structs; then elements of structs nested within those structs and so on.                               |
