...
| write.parquet.bloom-filter-adaptive-enabled         | false                       | Use parquet-mr adaptive bloom filter sizing (PARQUET-2254): evaluate candidate filters and pick the smallest that satisfies the actual NDV at the configured FPP, bounded by 'bloom-filter-max-bytes', instead of always allocating the full max-bytes buffer |
