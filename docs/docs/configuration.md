...
| cache.expiration-policy     | EXPIRE_AFTER_ACCESS| Determines the expiration strategy for catalog cache entries. Supported values include `EXPIRE_AFTER_ACCESS` and `EXPIRE_AFTER_WRITE`, which control whether entries expire based on last access time or creation time, respectively. |
