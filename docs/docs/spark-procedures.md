...
| `max-file-group-input-files` | Long.MAX_VALUE (unlimited) | Largest number of input files that should be rewritten in a single file group. This helps with breaking down the rewriting of very large partitions which may not be rewritable otherwise due to the resource constraints of the cluster. |
