...
| `min-overlap-depth` | (none) | Selects files for rewrite when at least this many files of the same partition cover a common point of the table sort key, regardless of their size. Without it, files that are large enough are never rewritten even if their sort key ranges sit entirely on top of each other, so a sorted rewrite can report success without improving clustering. The overlap is measured on the table sort order from data file bounds only, as reported by the `report-only` mode; truncated bounds make the measured depth an upper-bound estimate. Requesting this reads column bounds while planning, which increases the memory needed to plan the rewrite. Accepted for plain column sort orders only and rejected with `zorder()` or `hilbert()`, since the overlap is measured on a single sort column; the rewrite must also use the table sort order, since files selected by their overlap on the table key would otherwise keep that overlap and be selected again on every run. Not a useful signal when the first sort field has low cardinality, as the depth then approaches the file count regardless of layout. Unset by default, which leaves selection unchanged. |
