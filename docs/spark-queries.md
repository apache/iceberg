...
Content `0`  means it's a data file and content `1` a position-deletes file for [v2 format](../configuration/#reserved-table-properties). They also can be inspected individualy from `prod.db.table.data_files` and `prod.db.table.delete_files`.

To show a table's files for all tracked snapshots, query `prod.db.table.all_files`, `prod.db.table.all_data_files` and `prod.db.table.all_delete_files` respectively.
