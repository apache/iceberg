...
| validate-from-snapshot-id | null | If isolation level is set, identity of base snapshot from which to check concurrent write conflicts into a table. Should be the snapshot before any reads from the table. Can be obtained via [Table API](../../api#table-metadata) or [Snapshots table](../spark-queries#snapshots). If null, checks from the table's first un-expired snapshot. |
