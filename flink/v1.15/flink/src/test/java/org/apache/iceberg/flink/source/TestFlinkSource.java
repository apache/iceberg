...
public abstract class TestFlinkSource extends TestFlinkScan {
  ...
  protected List<Row> runWithOptions(Map<String, String> options) throws Exception {
    ...
    Optional.ofNullable(options.get("branch")).ifPresent(value -> builder.tag(value));
    Optional.ofNullable(options.get("start-tag")).ifPresent(value -> builder.tag(value));
    Optional.ofNullable(options.get("end-tag")).ifPresent(value -> builder.tag(value));
