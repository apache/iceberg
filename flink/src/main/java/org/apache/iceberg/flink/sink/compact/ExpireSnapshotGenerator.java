...
public class ExpireSnapshotGenerator extends AbstractStreamOperator<CommonControllerMessage>
  ...
  public void initializeState(StateInitializationContext context) throws Exception {
    ...
    this.retainMaxAgeMs = PropertyUtil.propertyAsLong(
        table.properties(),
        TableProperties.AUTO_EXPIRE_MAX_SNAPSHOT_AGE_MS,
        TableProperties.AUTO_EXPIRE_MAX_SNAPSHOT_AGE_MS_DEFAULT);

    this.retainLastValue = PropertyUtil
        .propertyAsInt(
            table.properties(),
            TableProperties.AUTO_EXPIRE_MIN_SNAPSHOTS_TO_KEEP,
            TableProperties.AUTO_EXPIRE_MIN_SNAPSHOTS_TO_KEEP_DEFAULT);
  }
  ...
  public void processElement(StreamRecord<EndCheckpoint> element) throws Exception {
    ...
    if (SmallFileUtil.shouldExpireSnapshot(table)) {
      ...
      table.updateProperties()
          .set(TableProperties.AUTO_EXPIRE_LAST_REMOVE_SNAPSHOTS_MS, Long.toString(currentTimeMillis))
          .commit();
      ...
      if (result != null) {
        ...
        int groupNums = PropertyUtil.propertyAsInt(
            table.properties(),
            TableProperties.AUTO_EXPIRE_SNAPSHOTS_GROUP_NUMS,
            TableProperties.AUTO_EXPIRE_SNAPSHOTS_GROUP_NUMS_DEFAULT);
        int indexOfFile = 0;
        int index = 0;
