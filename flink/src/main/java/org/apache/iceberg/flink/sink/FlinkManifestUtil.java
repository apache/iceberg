...
class FlinkManifestUtil {
  ...
  static DeltaManifests writeCompletedFiles(WriterResult result,
    ...
    if (result.dataFiles() != null && result.dataFiles().length > 0) {
      OutputFile dataManifestFile = outputFileSupplier.get();

      ManifestWriter<DataFile> writer =
          ManifestFiles.write(FORMAT_V2, spec, dataManifestFile, DUMMY_SNAPSHOT_ID);
      try (ManifestWriter<DataFile> closeableWriter = writer) {
        for (DataFile dataFile : result.dataFiles()) {
          closeableWriter.add(dataFile);
        }
      }
    ...
    if (result.deleteFiles() != null && result.deleteFiles().length > 0) {
      ...
      deleteManifest = writer.toManifestFile();
    }

    return new DeltaManifests(dataManifest, deleteManifest);
  }

  static WriterResult readCompletedFiles(DeltaManifests deltaManifests, FileIO io) throws IOException {
    ...
    if (deltaManifests.dataManifest() != null) {
      try (CloseableIterable<DataFile> dataFiles = ManifestFiles.read(deltaManifests.dataManifest(), io)) {
