...
public class TestFlinkIcebergSink {
  ...
  public void testTwoSinksInDisjointedDAG() throws Exception {
    ...
    final Table leftTable = SimpleDataUtil.createTable(leftTablePath, props, partitioned);
    final TableLoader leftTableLoader = TableLoader.fromHadoopTable(leftTablePath);

    final String rightTablePath = TEMPORARY_FOLDER.newFolder().getAbsolutePath().concat("/right");
    Assert.assertTrue("Should create the table path correctly.", new File(rightTablePath).mkdir());
    final Table rightTable = SimpleDataUtil.createTable(rightTablePath, props, partitioned);
    final TableLoader rightTableLoader = TableLoader.fromHadoopTable(rightTablePath);
    ...
    final List<Row> leftRows = createRows("left-");
    final DataStream<Row> leftStream = env.addSource(createBoundedSource(leftRows), ROW_TYPE_INFO)
    ...
    final List<Row> rightRows = createRows("right-");
    final DataStream<Row> rightStream = env.addSource(createBoundedSource(rightRows), ROW_TYPE_INFO)
