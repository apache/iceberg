...
public class TestIcebergFlinkSinkV2 extends TableTestBase {
  ...
  public TestIcebergFlinkSinkV2(String format, int parallelism, boolean partitioned) {
    super(FORMAT_V2);
  ...
  private void testChangeLogs(List<Integer> equalityFieldIds,
    ...
    // The first snapshot is generated by updateProperties.
    Assert.assertEquals("Should have the expected snapshot number", snapshots.size(), expectedSnapshotNum);
    ...
    for (int i = 0; i < expectedSnapshotNum; i++) {
      ...
      Assert.assertEquals("Should have the expected records for the checkpoint#" + (i + 1),
  ...
  private Row row(String rowKind, int id, String data) {
    if (rowKind.equals(RowKind.INSERT.shortString())) {
      return Row.ofKind(RowKind.INSERT, id, data);
    } else if (rowKind.equals(RowKind.DELETE.shortString())) {
      return Row.ofKind(RowKind.DELETE, id, data);
    } else if (rowKind.equals(RowKind.UPDATE_BEFORE.shortString())) {
      return Row.ofKind(RowKind.UPDATE_BEFORE, id, data);
    } else if (rowKind.equals(RowKind.UPDATE_AFTER.shortString())) {
      return Row.ofKind(RowKind.UPDATE_AFTER, id, data);
    } else {
