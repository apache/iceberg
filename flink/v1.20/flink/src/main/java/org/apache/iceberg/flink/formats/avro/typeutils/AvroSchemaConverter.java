...
public class AvroSchemaConverter {
  ...
  private static TypeInformation<?> convertToTypeInfo(
    ...
    switch (schema.getType()) {
      ...
      case MAP:
        return Types.MAP(
            Types.STRING, convertToTypeInfo(schema.getValueType(), legacyTimestampMapping));
      case UNION:
    ...
    switch (schema.getType()) {
      case RECORD:
        ...
        for (int i = 0; i < schemaFields.size(); i++) {
          ...
          fields[i] =
              DataTypes.FIELD(field.name(), convertToDataType(field.schema(), legacyMapping));
        }
        return DataTypes.ROW(fields).notNull();
      case ENUM:
        return DataTypes.STRING().notNull();
      case ARRAY:
        return DataTypes.ARRAY(convertToDataType(schema.getElementType(), legacyMapping)).notNull();
      case MAP:
      ...
      case UNION:
        ...
        if (schema.getTypes().size() == 2
          ...
          // use Kryo for serialization
          throw new UnsupportedOperationException("UNION with more than 2 types is not supported in Flink 1.20 backport.");
        }
        ...
        for (int i = 0; i < rowType.getFieldCount(); i++) {
          ...
          SchemaBuilder.GenericDefault<Schema> fieldBuilder =
              builder
                  ...
                  .type(
                      convertToSchema(
                          fieldType, rowName + "_" + fieldName, legacyTimestampMapping));
