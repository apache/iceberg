...
class TestIcebergFlinkManifest {
  private static final Configuration CONF = new Configuration();
  private @TempDir File folder;
  private Table table;
  ...
  public void before() throws IOException {
    ...
    assertThat(new File(tablePath).mkdir())
        .as("Should create the table directory correctly.")
        .isTrue();
    ...
    for (long checkpointId = 1; checkpointId <= 3; checkpointId++) {
      ManifestOutputFileFactory factory =
          FlinkManifestUtil.createOutputFileFactory(
              () -> table, table.properties(), "jobId", "operatorId");
      ...
      assertThat(result.deleteFiles()).as("Size of data file list are not equal").hasSize(10);
      assertThat(result.dataFiles()).as("Size of delete file list are not equal.").hasSize(10);
  ...
  @Test
  void testUserProvidedManifestLocation() throws IOException {
    long checkpointId = 1;
    File userProvidedFolder = folder;
    Map<String, String> props =
        ImmutableMap.of(FLINK_MANIFEST_LOCATION, userProvidedFolder.getAbsolutePath() + "///");
    ManifestOutputFileFactory factory =
        new ManifestOutputFileFactory(() -> table, props, "jobId", "operatorId");
    ...
    assertThat(deltaManifests.dataManifest()).as("Data manifest shouldn't be null").isNotNull();
    assertThat(deltaManifests.deleteManifest()).as("Delete manifest should be null").isNull();

    assertThat(userProvidedFolder.toPath())
        .as("The newly created manifest file should be located under the user provided directory")
        .isEqualTo(Paths.get(deltaManifests.dataManifest().path()).getParent());
    ...
    assertThat(dataFiles)
        .as("Size of data file list are not equal.")
        .hasSameSizeAs(result.dataFiles());
  ...
  public void testVersionedSerializer() throws IOException {
    long checkpointId = 1;
    ManifestOutputFileFactory factory =
        new ManifestOutputFileFactory(() -> table, table.properties(), "jobId", "operatorId");
    ...
    assertThat(versionedSerializeData).isEqualTo(versionedSerializeData2);
  }
  ...
  public void testCompatibility() throws IOException {
    ...
    assertThat(delta.deleteManifest()).as("Serialization v1 don't include delete files.").isNull();
    assertThat(delta.dataManifest())
        .as("Serialization v1 should not have null data manifest.")
        .isNotNull();

    assertThat(manifest).isEqualTo(delta.dataManifest());
