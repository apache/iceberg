...
public class TestBucketPartitioner {
    ...
    private TableTestStats extractTableTestStats() throws IOException {
        ...
        Map<Integer, List<Integer>> writersPerBucket = new HashMap<>();  // <BucketId, Set<WriterId>>
        Map<Integer, Integer> filesPerBucket = new HashMap<>();         // <BucketId, NumFiles>
        ...
        try (CloseableIterable<FileScanTask> fileScanTasks = table.newScan().planFiles()) {
            for (FileScanTask scanTask : fileScanTasks) {
                ...
                writersPerBucket.computeIfAbsent(bucketId, k -> new ArrayList<>());
    ...
    private TableTestStats sendAndAssertBucket0(int totalNumRows) throws Exception {
        List<Row> rows = generateRowsForBucketId(totalNumRows, 0, numBuckets);
    ...
    public void testSendRecordsToAllBucketsEvenly() throws Exception {
        ...
        List<Row> rows = new ArrayList<>(totalNumRows);
        List<Row> templateRows = new ArrayList<>(numBuckets);
        for (int i = 0; i < numBuckets; i++) {
            templateRows.addAll(generateRowsForBucketId(1, i, numBuckets));
        }
     ...
     * @return
     */
    private List<Row> generateRowsForBucketId(int numRows, int bucketId, int numBuckets) {
    ...
    /**
     * Utility method to generate a string value that will "hash" to a desired bucketId
     * @param bucketId
     * @param maxNumBuckets
     * @return
     */
    private String generateValueForBucketId(int bucketId, int maxNumBuckets) {
        String value = "";
        while (true) {
            String uuid = UUID.randomUUID().toString();
            // This is the same "hashing" mechanism used by Bucket.java
            if ((BucketUtil.hash(uuid) & Integer.MAX_VALUE) % maxNumBuckets == bucketId) {
                value = uuid;
                break;
            }
        }
    ...
    private static class TableTestStats {
        ...
        TableTestStats(int totalRecordCount,
    ...
    // Tests (all with parallelism = 8):
    // Test 1: N records sent to the same bucket across all partitions
    //  - 24 rows (3 batches w/overlap), 4 buckets
    //  - 30 rows (4 batches w/o overlap, uneven first bucket), 4 buckets
    // Test 2: N records of each bucket value, to all buckets, through the same partitions
    //  - 16 rows (2 writers per bucket, uneven last bucket), 3 buckets

    // About test parameters:
    //  - Constant parallelism (#TMs and #Slots)
    //  - Parallelism evenly divisible by #buckets
    //  - Parallelism not evenly divisible by #buckets
}
