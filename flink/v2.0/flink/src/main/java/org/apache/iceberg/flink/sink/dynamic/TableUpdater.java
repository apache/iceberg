...
class TableUpdater {
   ...
   */
  Tuple2<TableMetadataCache.SchemaCompareInfo, PartitionSpec> update(
    ...
    TableMetadataCache.SchemaCompareInfo newSchemaInfo =
  ...
  private TableMetadataCache.SchemaCompareInfo findOrCreateSchema(
      TableIdentifier identifier, Schema schema) {
    TableMetadataCache.SchemaCompareInfo fromCache = cache.schema(identifier, schema);
    if (fromCache.compareResult() != CompareSchemasVisitor.Result.SCHEMA_UPDATE_NEEDED) {
      ...
      switch (result) {
        case SAME:
          cache.update(identifier, table);
          return new TableMetadataCache.SchemaCompareInfo(
        ...
        case SCHEMA_UPDATE_NEEDED:
          ...
          try {
            updateApi.commit();
            cache.update(identifier, table);
            TableMetadataCache.SchemaCompareInfo comparisonAfterMigration =
                cache.schema(identifier, schema);
            Schema newSchema = comparisonAfterMigration.tableSchema();
            LOG.info("Table {} schema updated from {} to {}", identifier, tableSchema, newSchema);
            return comparisonAfterMigration;
          } catch (CommitFailedException e) {
            cache.invalidate(identifier);
            TableMetadataCache.SchemaCompareInfo newSchema = cache.schema(identifier, schema);
