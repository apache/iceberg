...
class DynamicTableUpdateOperator
  ...
  public DynamicRecordInternal map(DynamicRecordInternal data) throws Exception {
    ...
    TableMetadataCache.SchemaCompareInfo compareInfo = newData.f0;

    data.setSchema(compareInfo.tableSchema());
    data.setSpec(newData.f1);

    RowData newRowData = (RowData) newData.f0.converter().convert(data.rowData());
