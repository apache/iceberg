...
public class TestFlinkFormatModel extends BaseFormatModelTests<RowData> {
  ...
  protected boolean supports(Schema schema) {
    ...
    return schema.columns().stream().noneMatch(c -> c.type().typeId() == org.apache.iceberg.types.Type.TypeID.TIME || c.type().typeId() == org.apache.iceberg.types.Type.TypeID.UUID);
}
