 ...
 *
 * <p>Stale-index protection runs on two levels. Each key tracks the number the commands carrying
 * its stored positions were stamped with. The planner hands out a strictly higher number on every
 * rebuild of the index, so it serves both the equality test below and the ordering test:
 *
 * <ul>
