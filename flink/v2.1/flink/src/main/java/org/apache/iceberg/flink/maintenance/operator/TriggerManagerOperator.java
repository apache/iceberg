...
public class TriggerManagerOperator extends AbstractStreamOperator<Trigger>
  ...
  public void initializeState(StateInitializationContext context) throws Exception {
    ...
    if (context.isRestored()) {
      ...
      // collisions as well.
      operatorEventGateway.sendEventToCoordinator(new LockAcquiredEvent(lockId, Long.MAX_VALUE));
  ...
  public void onProcessingTime(long l) {
    ...
    checkAndFire(getProcessingTimeService().getCurrentProcessingTime());
  }
  ...
  private void handleLockAcquireResult(
    ...
    if (event.isLock()) {
      // Recover only send recovery trigger
  ...
  private void checkAndFire(long current) {
    this.taskToStart =
    ...
    operatorEventGateway.sendEventToCoordinator(new LockAcquiredEvent(lockId, current));
  }
