...
public class IcebergSink
  ...
  // Resolved at build time because flinkWriteConf is transient.
  ...
  private IcebergSink(
    ...
    this.writeParallelism = flinkWriteConf.writeParallelism();
  ...
  public static class Builder implements IcebergSinkBuilder<Builder> {
    ...
    /**
     * The REST prefix of the catalog behind this builder's loader, read while that loader is open.
     *
     * <p>The loader is opened here rather than left to {@link #checkAndGetTable}, which closes it
     * on its way out: the prefix is known only to a live catalog, so it has to be read while this
     * one is still up. Resolving it later would mean opening another catalog, on the submission
     * path, purely to report lineage.
     */
  ...
  /** The configured {@code write-parallelism}, or null to follow the input's parallelism. */
  ...
  private int resolveWriterParallelism(DataStream<RowData> input) {
    ...
    // encourage chaining.
    return Optional.ofNullable(writeParallelism).orElseGet(input::getParallelism);
  }
