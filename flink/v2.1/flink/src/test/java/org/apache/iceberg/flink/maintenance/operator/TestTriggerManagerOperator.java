...
class TestTriggerManagerOperator extends OperatorTestBase {
  ...
  void testCommitCount() throws Exception {
    ...
    try (OneInputStreamOperatorTestHarness<TableChange, Trigger> testHarness =
      ...
      addEventAndCheckResult(
          testHarness, TableChange.builder().commitCount(1).build(), mockGateway, 0);
      assertThat(testHarness.extractOutputValues()).hasSize(0);

      addEventAndCheckResult(
          testHarness, TableChange.builder().commitCount(2).build(), mockGateway, 1);
      lockAcquireFromCoordinator(operator);
      assertThat(testHarness.extractOutputValues()).hasSize(1);

      addEventAndCheckResult(
          testHarness, TableChange.builder().commitCount(10).build(), mockGateway, 2);
      lockAcquireFromCoordinator(operator);
      assertThat(testHarness.extractOutputValues()).hasSize(2);

      // No trigger in this case
      addEventAndCheckResult(
          testHarness, TableChange.builder().commitCount(1).build(), mockGateway, 2);
      assertThat(testHarness.extractOutputValues()).hasSize(2);

      addEventAndCheckResult(
          testHarness, TableChange.builder().commitCount(2).build(), mockGateway, 3);
      lockAcquireFromCoordinator(operator);
  ...
  void testDataFileCount() throws Exception {
    MockOperatorEventGateway mockGateway = new MockOperatorEventGateway();
    TriggerManagerOperator operator =
        createOperator(new TriggerEvaluator.Builder().dataFileCount(3).build(), mockGateway);
    try (OneInputStreamOperatorTestHarness<TableChange, Trigger> testHarness =
        createHarness(operator)) {
      addEventAndCheckResult(
          testHarness, TableChange.builder().dataFileCount(1).build(), mockGateway, 0);
      assertThat(mockGateway.getEventsSent()).isEmpty();
      assertThat(testHarness.extractOutputValues()).hasSize(0);

      addEventAndCheckResult(
          testHarness, TableChange.builder().dataFileCount(2).build(), mockGateway, 1);
      lockAcquireFromCoordinator(operator);
      assertThat(testHarness.extractOutputValues()).hasSize(1);

      addEventAndCheckResult(
          testHarness, TableChange.builder().dataFileCount(10).build(), mockGateway, 2);
      lockAcquireFromCoordinator(operator);
      assertThat(testHarness.extractOutputValues()).hasSize(2);

      // No trigger in this case
      addEventAndCheckResult(
          testHarness, TableChange.builder().dataFileCount(1).build(), mockGateway, 2);
      ...
      addEventAndCheckResult(
          testHarness, TableChange.builder().dataFileCount(2).build(), mockGateway, 3);
      lockAcquireFromCoordinator(operator);
      assertThat(testHarness.extractOutputValues()).hasSize(3);
    }
  }
  ...
  void testTimeout() throws Exception {
    ...
    TriggerManagerOperator operator =
        createOperator(
            new TriggerEvaluator.Builder().timeout(Duration.ofSeconds(1)).build(), mockGateway);
    try (OneInputStreamOperatorTestHarness<TableChange, Trigger> testHarness =
        createHarness(operator)) {
  ...
  void testStateRestore() throws Exception {
    ...
    MockOperatorEventGateway mockGateway = new MockOperatorEventGateway();
    TriggerManagerOperator operator =
        createOperator(new TriggerEvaluator.Builder().commitCount(2).build(), mockGateway);
    try (OneInputStreamOperatorTestHarness<TableChange, Trigger> testHarness =
        createHarness(operator)) {
    ...
    MockOperatorEventGateway newMockGateway = new MockOperatorEventGateway();
    TriggerManagerOperator newOperator =
        createOperator(new TriggerEvaluator.Builder().commitCount(2).build(), newMockGateway);
    try (OneInputStreamOperatorTestHarness<TableChange, Trigger> testHarness =
  ...
  void testLockCheckDelay() throws Exception {
    ...
    try (OneInputStreamOperatorTestHarness<TableChange, Trigger> testHarness =
      ...
      // lock has been released, no new trigger yet
      addEventAndCheckResult(
          testHarness, TableChange.builder().commitCount(2).build(), mockGateway, 2);
      unLockAcquireFromCoordinator(operator);
      assertThat(mockGateway.getEventsSent()).hasSize(2);
      assertThat(testHarness.extractOutputValues()).hasSize(1);
      ...
      // Check that the trigger fired after the delay
      testHarness.setProcessingTime(currentTime + DELAY);
      assertThat(mockGateway.getEventsSent()).hasSize(3);
      lockAcquireFromCoordinator(operator);
      assertThat(testHarness.extractOutputValues()).hasSize(2);
    }
  }
  ...
  private void addEventAndCheckResult(
      ...
      TableChange event,
      MockOperatorEventGateway mockGateway,
      int expectedSize)
      throws Exception {
    ++processingTime;
    testHarness.setProcessingTime(processingTime);
    testHarness.processElement(event, processingTime);
    assertThat(mockGateway.getEventsSent()).hasSize(expectedSize);
  }
