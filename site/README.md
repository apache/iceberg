...
> [lint](dev/lint.sh): Fix markdown file styles.
...
It will also scan all markdown files and fail the build on any style issues. To fix style issues, run the `lint` recipe.
```sh
make lint
```
