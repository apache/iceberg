...
Most pull requests can be merged once a committer is satisfied with the code in the PR. Before 
merging all review comments should be addressed either by making changes or agreeing the request is out of scope for the PR. For larger changes or additions to public
...
a reviewer does not think the PR should be merged and that immediate changes by the PR author cannot
...
if a change should go through the  [Iceberg improvement proposal](#apache-iceberg-improvement-proposals) process.
