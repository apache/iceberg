...
This is the first post in a series on Variant in Apache Iceberg. It covers what Variant is, why it exists, and how it fits into an Iceberg table. Later posts cover shredding, the technique that stores frequently accessed Variant fields as typed, columnar data. Variant is stored in Parquet, Avro, and ORC; shredding is currently available only in Parquet.

<!-- more -->

## What is Variant?

Variant is a self-describing type for semi-structured data. A single Variant column can hold objects, arrays, and primitive values.
...
Variant was added to the Iceberg type system in the v3 spec, alongside other v3 extensions such as nanosecond timestamps and the `geometry` and `geography` types. The spec places it in its own category: `variant` is "neither a primitive type nor a nested type." It is richer than a primitive, yet has no fixed, declared shape the way a struct or list does.
...
2. A **Variant object** is a collection of string-keyed fields whose values are themselves Variant values. Unlike an Iceberg struct, its fields are not a fixed, named set of typed columns.
...
With Spark SQL, you store heterogeneous events in a single `VARIANT` column and read fields back with `variant_get`. A Variant column requires a v3 table:
...
- **Tracking issue:** [Variant Data Type Support (#10392)](https://github.com/apache/iceberg/issues/10392)