...
public class HiveIcebergOutputCommitter extends OutputCommitter {
  ...
  public void commitTask(TaskAttemptContext originalContext) throws IOException {
    ...
    for (String table : outputs.keySet()) {
      HiveIcebergRecordWriter writer = writers.get(table);
      DataFile[] closedFiles = writer != null ? writer.dataFiles() : new DataFile[0];
      String fileForCommitLocation = generateFileForCommitLocation(outputs.get(table), jobConf, attemptID.getJobID(),
          attemptID.getTaskID().getId());

      // Creating the file containing the data files generated by this task for this table
      FileIO io = HiveIcebergStorageHandler.table(context.getJobConf()).io();
      createFileForCommit(closedFiles, fileForCommitLocation, io);
    }
  ...
  public void commitJob(JobContext originalContext) throws IOException {
    ...
    Map<String, String> outputs =
        SerializationUtil.deserializeFromBase64(jobContext.getJobConf().get(InputFormatConfig.OUTPUT_TABLES));
    FileIO io = HiveIcebergStorageHandler.table(jobContext.getJobConf()).io();

    outputs.forEach((table, location) -> commitTable(io, jobContext, table, location));
  ...
  public void abortJob(JobContext originalContext, int status) throws IOException {
    ...
    for (String table : outputs.keySet()) {
      LOG.info("Cleaning job for table {}", jobContext.getJobID(), table);

      List<DataFile> dataFiles = dataFiles(outputs.get(table), jobContext, io, false);
   ...
   */
  private void commitTable(FileIO io, JobContext jobContext, String name, String location) {
  ...
  private void cleanup(JobContext jobContext) throws IOException {
    ...
    for (String table : outputs.keySet()) {
      String tableLocation = generateJobLocation(outputs.get(table), jobConf, jobContext.getJobID());
   ...
   */
  private static List<DataFile> dataFiles(String location, JobContext jobContext, FileIO io, boolean throwOnFailure) {
    JobConf conf = jobContext.getJobConf();
