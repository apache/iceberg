...
public class HiveIcebergOutputCommitter extends OutputCommitter {
  ...
  public void commitTask(TaskAttemptContext originalContext) throws IOException {
    ...
    for (String location : outputs.keySet()) {
      HiveIcebergRecordWriter writer = writers.get(location);
      DataFile[] closedFiles = writer != null ? writer.dataFiles() : new DataFile[0];
      String fileForCommitLocation =
          generateFileForCommitLocation(location, jobConf, attemptID.getJobID(), attemptID.getTaskID().getId());
  ...
  public void commitJob(JobContext originalContext) throws IOException {
    ...
    for (String location : outputs.keySet()) {
      commitTable(io, jobContext, outputs.get(location), location);
    }
  ...
  private void cleanup(JobContext jobContext) throws IOException {
    ...
    for (String location : outputs.keySet()) {
      String tableLocation = generateJobLocation(location, jobConf, jobContext.getJobID());
