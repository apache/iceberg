...
public class TestCosOutputFile {
  private final COS cosClient = TestUtility.cos();
  ...
  public void testWriteFile() throws IOException {
    ...
    OutputFile out = CosOutputFile.fromLocation(cosClient, uri.location(), cosProperties);
    try (OutputStream os = out.create();
        InputStream is = new ByteArrayInputStream(data)) {
      ByteStreams.copy(is, os);
    }

    assertThat(cosClient.doesObjectExist(uri.bucket(), uri.key())).as("Cos file should exist").isTrue();
    assertThat(cosDataLength(uri)).as("Object length should match").isEqualTo(dataSize);

    assertThat(data).as("Object content should match").containsExactly(cosDataContent(uri, dataSize));
  }

  @Test
  public void testFromLocation() {
    assertThatThrownBy(() -> CosOutputFile.fromLocation(cosClient, null, cosProperties))
  ...
  public void testCreateOrOverwrite() throws IOException {
    ...
    assertThat(cosDataLength(uri)).as(String.format("Should overwrite object length from %d to %d", dataSize, expectSize))
                    .isEqualTo(expectSize);
    assertThat(cosDataContent(uri, expectSize)).as("Should overwrite object content").containsExactly(expect);
  }

  @Test
  public void testLocation() {
    CosURI uri = randomURI();
    OutputFile out = new CosOutputFile(cosClient, uri, cosProperties, MetricsContext.nullMetrics());
  ...
  public void testToInputFile() throws IOException {
    ...
    OutputFile out =
        new CosOutputFile(cosClient, randomURI(), cosProperties, MetricsContext.nullMetrics());
    try (OutputStream os = out.create();
        InputStream is = new ByteArrayInputStream(data)) {
      ByteStreams.copy(is, os);
    }
  ...
  private long cosDataLength(CosURI uri) {
    return cosClient.getObject(uri.bucket(), uri.key()).getObjectMetadata().getContentLength();
  }

  private byte[] cosDataContent(CosURI uri, int dataSize) throws IOException {
    try (InputStream is = cosClient.getObject(uri.bucket(), uri.key()).getObjectContent()) {
      byte[] actual = new byte[dataSize];
