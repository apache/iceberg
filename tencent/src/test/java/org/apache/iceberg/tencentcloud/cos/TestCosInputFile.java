...
public class TestCosInputFile {
  ...
  public void testCosInputFile() {
    ...
    Assertions.assertThatThrownBy(
            () -> new CosInputFile(cosMock, uri, cosProperties, -1, MetricsContext.nullMetrics()))
  ...
  public void testExists() {
    ...
    InputFile inputFile =
        new CosInputFile(cosMock, uri, cosProperties, MetricsContext.nullMetrics());
  ...
  private void verifyLength(COS cosClientMock, CosURI uri, byte[] data, boolean isCache) {
    ...
    if (isCache) {
      inputFile =
          new CosInputFile(
              cosClientMock, uri, cosProperties, data.length, MetricsContext.nullMetrics());
    } else {
      inputFile = new CosInputFile(cosClientMock, uri, cosProperties, MetricsContext.nullMetrics());
    }
    inputFile.getLength();
    assertThat(inputFile.getLength()).as("Should have expected file length").isEqualTo(data.length);
  }
