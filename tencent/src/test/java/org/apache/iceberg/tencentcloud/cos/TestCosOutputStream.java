...
public class TestCosOutputStream {
  ...
  private final COS cosMock = mock(COS.class, delegatesTo(cosClient));
  ...
  private void writeAndVerify(COS mock, CosURI uri, byte[] data, boolean arrayWrite)
    ...
    assertThat(cosClient.doesObjectExist(uri.bucket(), uri.key())).as("Cos object should exist").isTrue();
    assertThat(cosClient.getObject(uri.bucket(), uri.key()).getObjectMetadata().getContentLength())
            .as( "Object length").isEqualTo(data.length);
    assertThat(data).as("Object content").containsExactly(cosDataContent(uri, data.length));

    // Verify all staging files are cleaned up.
      assertThat(Files.list(Paths.get(props.cosTmpDir())).count()).as("Staging files should clean up").isEqualTo(0);
  }
