 ...
 *       be used.
 *   <li>{@code gcp.auth.credentials-key}: JSON string of a service account credential. If neither
    ...
    try {
      if (useCredentialsPath) {
        ...
        try (InputStream credentialsStream =
            new ByteArrayInputStream(credentialsKey.getBytes(StandardCharsets.UTF_8))) {
          this.credentials = GoogleCredentials.fromStream(credentialsStream).createScoped(scopes);
        }
      } else {
