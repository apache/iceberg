...
public class GenericDataOrcWriter implements OrcValueWriter<GenericData.Record> {
  ...
  public static OrcValueWriter<GenericData.Record> buildWriter(TypeDescription fileSchema) {
    return new GenericDataOrcWriter(fileSchema);
  }

  @SuppressWarnings("unchecked")
  @Override
  public void write(GenericData.Record value, VectorizedRowBatch output) throws IOException {
    int row = output.size++;
  ...
  static class UUIDConverter implements Converter<UUID> {
    ...
    public void addValue(int rowId, UUID data, ColumnVector output) {
      bytesConverter.addValue(rowId, data != null ? asBytes(data) : null, output);
    }
  }
  ...
  static class FixedConverter implements Converter<GenericData.Fixed> {
    ...
    @Override
    public void addValue(int rowId, GenericData.Fixed data, ColumnVector output) {
      bytesConverter.addValue(rowId, data != null ? data.bytes() : null, output);
    }
  }
    ...
    public void addValue(int rowId, BigDecimal data, ColumnVector output) {
      ...
      if (data == null) {
        ...
        ((DecimalColumnVector) output).vector[rowId]
            .set(HiveDecimal.create(data, false), this.precision, this.scale);
      }
    }
  }
  ...
  static class StructConverter implements Converter<GenericData.Record> {
    ...
    @SuppressWarnings("unchecked")
    public void addValue(int rowId, GenericData.Record data, ColumnVector output) {
      if (data == null) {
        output.noNulls = false;
        output.isNull[rowId] = true;
      } else {
        ...
        for (int c = 0; c < children.length; ++c) {
          children[c].addValue(rowId, data.get(c), cv.fields[c]);
        }
      }
    }
  }
  ...
  private static Converter buildConverter(TypeDescription schema) {
    switch (schema.getCategory()) {
      ...
      case DATE:
      case INT:
        if ("TIME".equals(schema.getAttributeValue("iceberg.integer-type"))) {
      ...
      case BINARY:
        if ("UUID".equals(schema.getAttributeValue("iceberg.binary-type"))) {
          return new UUIDConverter();
        } else if ("FIXED".equals(schema.getAttributeValue("iceberg.binary-type"))) {
