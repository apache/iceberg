...
public class BatchTest extends BaseTest {
  ...
  public void runPipeline(FileFormat fileFormat) {
    ...
    p.getCoderRegistry().registerCoderForClass(GenericRecord.class, AvroCoder.of(Cat.getClassSchema()));
    ...
    TableIdentifier name = TableIdentifier.of("default", "test_batch_" + fileFormat.name().toLowerCase());

    records.apply(IcebergIO.write()
        .withSchema(icebergSchema)
        .withTableIdentifier(name)
        .withConfigProperties(
          new HashMap<String, String>() {{
            put(TableProperties.DEFAULT_FILE_FORMAT, fileFormat.name());
          }}));
  ...
  public void runPipelineSpecific(FileFormat fileFormat) {
    ...
    p.getCoderRegistry().registerCoderForClass(Cat.class, AvroCoder.of(Cat.getClassSchema()));

    PCollection<Cat> cats = p.apply(Create.of(specificCats));

    org.apache.iceberg.Schema icebergSchema = AvroSchemaUtil.toIceberg(Cat.getClassSchema());
    TableIdentifier name = TableIdentifier.of("default", "test_specific_batch_" + fileFormat.name().toLowerCase());

    cats.apply(IcebergIO.write()
            .withSchema(icebergSchema)
            .withTableIdentifier(name)
            .withConfigProperties(
                    new HashMap<String, String>() {{
