...
public class HiveCatalogHelper {
    ...
    public static synchronized Table loadOrCreateTable(HiveCatalog hiveCatalog, TableIdentifier tableIdentifier, Schema schema) {
        try {
            return hiveCatalog.loadTable(tableIdentifier);
        } catch (NoSuchTableException noSuchTableException) {
            try {
                // If it doesn't exist, we just create the table
                return hiveCatalog.createTable(tableIdentifier, schema);
