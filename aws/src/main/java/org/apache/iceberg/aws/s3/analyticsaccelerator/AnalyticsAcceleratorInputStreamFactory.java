...
import org.apache.iceberg.aws.s3.S3ObjectMetadataUtil;
...
public class AnalyticsAcceleratorInputStreamFactory implements S3InputStreamFactory {
  ...
  public SeekableInputStream createStream(
      S3Client client, S3URI uri, S3FileIOProperties properties, MetricsContext metrics) {
    try {
      final OpenStreamInformation openStreamInformation =
          OpenStreamInformation.builder()
              .objectMetadata(
                  S3ObjectMetadataUtil.getAnalyticsAcceleratorMetadata(client, uri, properties))
              .build();
