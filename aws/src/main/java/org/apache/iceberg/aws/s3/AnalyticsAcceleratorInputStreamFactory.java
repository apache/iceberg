...
public class AnalyticsAcceleratorInputStreamFactory implements S3InputStreamFactory {
    ...
    try {
      return new AnalyticsAcceleratorInputStream(
          seekableInputStreamFactory.createStream(
              ...
              buildOpenStreamInformation(metadata)));
    } catch (IOException e) {
  ...
  private OpenStreamInformation buildOpenStreamInformation(HeadObjectResponse metadata) {
    final ObjectMetadata objectMetadata =
        ObjectMetadata.builder()
            .contentLength(metadata.contentLength())
            .etag(metadata.eTag())
            .build();
    return OpenStreamInformation.builder().objectMetadata(objectMetadata).build();
  }
}
