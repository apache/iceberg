...
class LoadTableResult(BaseModel):
    ...
    - `rest-scan-planning-mode`: Controls the scan planning behavior. Valid values are:
      - `none` (default): Server-side scan planning is not supported or allowed. Client must use client-side planning.
      - `optional`: Client can choose between client-side or server-side planning based on its capabilities and preferences.
      - `required`: Server-side planning is required. Client must not use client-side planning.
