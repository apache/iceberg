...
class LoadTableResult(BaseModel):
    ...
    - `scan-planning-mode`: Controls scan planning behavior for table operations. This property can be configured by:
      ...
      **Configuration Precedence**: Client config > Server config > Default (`client-preferred`)

      **Valid values**:
      - `client-only`: MUST use client-side planning. Fails if paired with server's `catalog-only`.
      - `client-preferred` (default): Prefer client-side planning but flexible.
      - `catalog-preferred`: Prefer server-side planning but flexible. Falls back to client if server doesn't support planning endpoints.
      - `catalog-only`: MUST use server-side planning. Requires server support. Fails if paired with client's `client-only`.

    ### Scan Planning Negotiation

    When both client and server provide `scan-planning-mode` configuration, the final planning decision is negotiated based on the following rules:

    **Negotiation Rules:**
    - **Incompatible requirements**: `client-only` + `catalog-only` = **FAIL**
    - **ONLY beats PREFERRED**: When one side has "ONLY" and the other has "PREFERRED", the ONLY requirement wins (inflexible beats flexible)
