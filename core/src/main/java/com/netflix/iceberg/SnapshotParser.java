...
public class SnapshotParser {
  ...
  static Snapshot fromJson(TableOperations ops, JsonNode node) {
    ...
    if (node.has(MANIFEST_LIST)) {
      ...
      return new BaseSnapshot(ops, versionId, parentId, timestamp, ops.fileIo().newInputFile(manifestList));
      ...
      List<ManifestFile> manifests = Lists.transform(JsonUtil.getStringList(MANIFESTS, node),
          location -> new GenericManifestFile(ops.fileIo().newInputFile(location), 0));
