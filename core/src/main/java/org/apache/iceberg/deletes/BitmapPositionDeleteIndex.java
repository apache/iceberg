...
class BitmapPositionDeleteIndex implements PositionDeleteIndex {
  ...
  public void delete(long posStart, long posEnd) {
    bitmap.addRange(posStart, posEnd);
  }
   ...
   *
   * <ul>
   *   <li>The length of the bitmap and magic bytes stored as 4 bytes (big-endian).
   ...
   *   <li>A CRC-32 checksum of the bitmap and magic bytes as 4-bytes (big-endian).
   * </ul>
   *
  ...
  public ByteBuffer serialize() {
    bitmap.runOptimize(); // compress the bitmap before serializing
    ...
    serializeBitmapData(bytes, bitmap, bitmapDataLength);
  ...
  private static int computeBitmapDataLength(RoaringPositionBitmap bitmap) {
    long bitmapDataLength = MAGIC_NUMBER_SIZE_BYTES + bitmap.serializedSizeInBytes();
    long bufferSize = LENGTH_SIZE_BYTES + bitmapDataLength + CRC_SIZE_BYTES;
    Preconditions.checkState(bufferSize <= Integer.MAX_VALUE, "Can't serialize index > 2GB");
    return (int) bitmapDataLength;
  }

  // serializes the bitmap data (magic bytes + bitmap) using the little-endian byte order
  private static void serializeBitmapData(
      byte[] bytes, RoaringPositionBitmap bitmap, int bitmapDataLength) {
  ...
  private static int readBitmapDataLength(ByteBuffer buffer, DeleteFile deleteFile) {
    ...
    Preconditions.checkArgument(
        bitmapDataLength == expectedLength,
        "Invalid bitmap data length %s, expected %s",
        bitmapDataLength,
        expectedLength);
    return bitmapDataLength;
  }

  // validates magic bytes and deserializes the bitmap
  private static RoaringPositionBitmap deserializeBitmap(byte[] bytes, int bitmapDataLength) {
    ByteBuffer bitmapData = pointToBitmapData(bytes, bitmapDataLength);
    ...
    Preconditions.checkArgument(
        ...
        magicNumber,
        MAGIC_NUMBER);
    return RoaringPositionBitmap.deserialize(bitmapData);
  }
