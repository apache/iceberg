...
import org.apache.iceberg.LazyWork;
...
/**
 * exchange T Type to R type by LazyWork
 * */
public class ParallelIterableLazy<T, R> extends CloseableGroup implements CloseableIterable<R> {
  private static final Logger LOG = LoggerFactory.getLogger(ParallelIterable.class);
  private final Iterable<T> iterables;
  private final ExecutorService workerPool;

  public ParallelIterableLazy(Iterable<T> iterables,
  ...
  public CloseableIterator<R> iterator() {
    ParallelIterator<T, R> iter = new ParallelIterator<>(iterables, workerPool);
    addCloseable(iter);
    return iter;
  }

  private static class ParallelIterator<T, R> implements CloseableIterator<R> {
    ...
    private ParallelIterator(Iterable<T> iterables,
                             ExecutorService workerPool) {
      this.tasks = Iterables.transform(iterables, iterable ->
              (Runnable) () -> {
                long start = System.currentTimeMillis();
                if (iterable instanceof LazyWork) {
                  CloseableIterable<R> closeableIterable = ((LazyWork<T, CloseableIterable<R>>) iterable).run();
                  for (R res : closeableIterable) {
                    queue.add(res);
                  }
                }
                long end = System.currentTimeMillis();
                long mis = end - start;
                LOG.info("trino_lazy_read done , read time: {}", mis);
              }).iterator();
