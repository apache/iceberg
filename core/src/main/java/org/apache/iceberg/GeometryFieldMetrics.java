 ...
 * values, so the writer instead scans each value's coordinates with a {@link GeometryBoundsBuilder}
 * and accumulates a two-dimensional bounding box. The resulting {@link FieldMetrics} carries the
 * box's lower and upper corners as {@link GeospatialBound} points, which serialize through the
...
public class GeometryFieldMetrics extends FieldMetrics<GeospatialBound> {
  ...
  private GeometryFieldMetrics(
    ...
    super(id, valueCount, 0L, -1L, lowerBound, upperBound, type);
  }

  public static class Builder {
    private final int id;
    private final Type geometryType;
    private final GeometryBoundsBuilder boundsBuilder = new GeometryBoundsBuilder();
    ...
    public void addValue(ByteBuffer wkb) {
      this.valueCount += 1;
      boundsBuilder.addValue(wkb);
    }

    public GeometryFieldMetrics build() {
      // build() returns null when a dimension has no value or a value could not be bounded, which
      // leaves the field with counts but no bounds
      ...
      return new GeometryFieldMetrics(id, valueCount, lowerBound, upperBound, geometryType);
    }
  }
}
