...
public abstract class ManifestWriter<F extends ContentFile<F>> implements FileAppender<F> {
  ...
  // separately from the per-status counters above because v4+ status counts
  ...
  static class V4Writer extends ManifestWriter<DataFile> {
    ...
    private final TrackedFileAdapters.DataTrackedFile trackedFile;
    ...
    protected ManifestEntry<DataFile> prepare(ManifestEntry<DataFile> entry) {
      wrapTrackedFile(trackedFile, entry);
      return writerEntry.wrap(entry, trackedFile);
    }

    private static void wrapTrackedFile(
        TrackedFileAdapters.DataTrackedFile wrapper, ManifestEntry<DataFile> entry) {
  ...
  static class V4DeleteWriter extends ManifestWriter<DeleteFile> {
    ...
    private final TrackedFileAdapters.EqualityDeleteTrackedFile trackedFile;
    ...
    private static void wrapTrackedFile(
        TrackedFileAdapters.EqualityDeleteTrackedFile wrapper, ManifestEntry<DeleteFile> entry) {
      ...
      switch (entry.status()) {
        case ADDED:
          wrapper.wrap(
              file,
              new TrackingStruct(
                  EntryStatus.ADDED, entry.snapshotId(), null, null, null, null, null, null));
          return;
        case EXISTING:
          wrapper.wrap(
              file,
