...
interface TokenExchangeConfig {
  ...
  /**
   * The type of the subject token. Must be a valid URN.
   *
   * <p>Optional. If absent, defaults to {@code urn:ietf:params:oauth:token-type:access_token}.
   *
  ...
  /**
   * The type of the actor token. Must be a valid URN.
   *
   * <p>Optional. If absent, defaults to {@code urn:ietf:params:oauth:token-type:access_token} when
   * an actor token is used.
   *
   ...
   *
   * <p>Defaults to an empty list. Can be a single value or a comma-separated list of values.
   */
  List<URI> resources();
