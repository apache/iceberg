...
public class HTTPClient extends BaseHTTPClient {
  ...
  @VisibleForTesting
  static Duration parseKeyLifetime(String lifetime) {
    if (lifetime == null) {
      return null;
    }
    try {
      Duration parsed = Duration.parse(lifetime);
      if (parsed.isZero() || parsed.isNegative()) {
        // Non-positive window: never retry keyed requests (null would instead disable the bound).
        return Duration.ZERO;
      }
      if (parsed.compareTo(Duration.ofMillis(Long.MAX_VALUE)) > 0) {
        LOG.warn("Ignoring out-of-range idempotency-key lifetime: {}", lifetime);
        return null;
      }
      return parsed;
