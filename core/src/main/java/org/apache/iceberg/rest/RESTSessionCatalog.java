...
public class RESTSessionCatalog extends BaseViewSessionCatalog
  ...
  public Table loadTable(SessionContext context, TableIdentifier identifier) {
    ...
    try {
      ...
      if (response == null) {
        ...
        return cloneTable(cachedTable.table());
      }
        ...
        try {
          ...
          if (response == null) {
            ...
            return MetadataTableUtils.createMetadataTableInstance(
                cloneTable(cachedTable.table()), metadataType);
          }
  ...
  // Creates a new table object based on 'originalTable' where the underlying ops is also a
  // cloned object from the one in 'originalTable', but everything else (e.g. FileIO, HTTPClient,
  // TableMetadata) are the same objects.
  private BaseTable cloneTable(BaseTable originalTable) {
    return new BaseTable(
        new RESTTableOperations((RESTTableOperations) originalTable.operations()),
        originalTable.name(),
        originalTable.reporter());
  }
