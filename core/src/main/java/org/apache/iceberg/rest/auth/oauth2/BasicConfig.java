...
interface BasicConfig {
   ...
   *
   * <p>Optional. Either this property or {@link #TOKEN_ENDPOINT} must be set, unless a {@linkplain
  ...
  /**
   * URL of the OAuth2 token endpoint. If not set, the token endpoint will be discovered from the
   * {@link #ISSUER_URL issuer URL}, using the OpenID Connect Discovery metadata published by the
   * issuer.
   *
   * <p>Optional. Either this property or {@link #ISSUER_URL} must be set, unless a {@linkplain
   * #TOKEN static token} is provided.
   */
   ...
   * </ul>
   *
   * Defaults to {@link #DEFAULT_GRANT_TYPE}.
   */
  @Value.Default
   ...
   *
   * <p>Optional. Required unless a {@linkplain #TOKEN static token} is provided.
   */
   ...
   * </ul>
   *
   * Defaults to {@link #DEFAULT_CLIENT_AUTH}.
   */
  @Value.Default
   ...
   *
   * <p>Optional. If absent, no scopes are sent.
   *
   ...
   * parameters that are not covered by the standard OAuth2 specification. Defaults to an empty map.
   *
  ...
  /**
   * The token acquisition timeout. Defaults to {@link #DEFAULT_TIMEOUT}. The default timeout is
   * intentionally large, in order to accommodate for long-running flows that require human
   * intervention (e.g. Authorization Code flow).
   *
