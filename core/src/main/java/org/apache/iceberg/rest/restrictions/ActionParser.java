...
public class ActionParser {
  ...
  public static IcebergFunction<?, ?> fromJson(JsonNode node) {
    ...
    // Unknown action types are preserved as UnknownFunction so older clients don't break when the
    // spec evolves. Enforcement code (engine rules) must fail closed on UnknownFunction, since
    // silently skipping an unknown mask would leak unmasked data.
    return IcebergFunctions.fromString(actionType, fieldId);
  }
}
