...
public abstract class BaseManifestWriterFactory implements Serializable {
  ...
  protected ManifestWriter<DataFile> newManifestWriter() {
    return ManifestFiles.write(formatVersion, getSpec(), newOutputFile(), null);
  }

  protected ManifestWriter<DeleteFile> newDeleteManifestWriter() {
    return ManifestFiles.writeDeleteManifest(formatVersion, getSpec(), newOutputFile(), null);
  }

  protected OutputFile newOutputFile() {
    String fileName = FileFormat.AVRO.addExtension("optimized-m-" + UUID.randomUUID());
    return getFileIO().newOutputFile(new Path(outputLocation, fileName).toString());
  }

  protected abstract PartitionSpec getSpec();

  protected abstract FileIO getFileIO();
