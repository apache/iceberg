...
abstract class ManifestFilterManager<F extends ContentFile<F>> {
  ...
  private Set<String> dataFilePathsWithOrphanedDVs = Sets.newHashSet();
  ...
  protected void removeOrphanedDeletesFor(Set<DataFile> dataFiles) {
    this.dataFilePathsWithOrphanedDVs =
  ...
  private boolean canContainDroppedFiles(ManifestFile manifest) {
    ...
    } else if (!dataFilePathsWithOrphanedDVs.isEmpty()) {
      return true;
    }
    ...
    for (ManifestEntry<F> entry : reader.liveEntries()) {
      ...
      boolean markedForDelete =
          deletePaths.contains(file.location())
              ...
              || (isDelete && isOrphanedDV((DeleteFile) file));
  ...
  private boolean isOrphanedDV(DeleteFile file) {
    return null != file
        && null != file.referencedDataFile()
        && dataFilePathsWithOrphanedDVs.contains(file.referencedDataFile());
  }
      ...
      try {
        reader
            ...
            .forEach(
                entry -> {
                  F file = entry.file();
                  boolean isOrphanedDV = isDelete && isOrphanedDV((DeleteFile) entry.file());
                  boolean markedForDelete =
                      deletePaths.contains(file.location())
                          || deleteFiles.contains(file)
                          || dropPartitions.contains(file.specId(), file.partition())
