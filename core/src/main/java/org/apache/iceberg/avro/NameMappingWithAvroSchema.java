...
public class NameMappingWithAvroSchema extends AvroSchemaWithDerivedTypeVisitor<MappedFields> {
  ...
  @Override
  public MappedFields record(Types.StructType iStruct, Schema record, List<String> names, List<MappedFields> fieldResults) {
    ...
    if (AvroSchemaUtil.isOptionSchema(union)) {
      ...
      for (Schema option : union.getTypes()) {
        if (option.getType() != Schema.Type.NULL) {
          // Check if current option is a named type, i.e., a RECORD, ENUM, or FIXED type. If so, use the record name
          ...
          if (option.getType() == Schema.Type.RECORD ||
            ...
            fields.add(MappedField.of(iStruct.fields().get(i).fieldId(), option.getName(), optionResults.get(i)));
          } else {
            fields.add(MappedField.of(iStruct.fields().get(i).fieldId(), option.toString(), optionResults.get(i)));
          }
          // Both iStruct and optionResults do not contain an entry for the NULL type, so we need to increment i only
  ...
  @Override
  public MappedFields map(Types.MapType iMap, Schema map, MappedFields keyResult, MappedFields valueResult) {
