...
public class EnvelopeMetadataParser {
  ...
  private static final String WRAPPED_KEK = "wrapped-kek";
  private static final String WRAPPED_DEK = "wrapped-dek";
  private static final String IV = "iv";
  private static final String ALGORITHM = "algorithm";
  private static final String PROPERTIES = "properties";
  private static final String AAD_PREFIX = "aad-prefix";
  ...
  public static void toJson(EnvelopeMetadata metadata, JsonGenerator generator) throws IOException {
    ...
    JsonUtil.writeStringIfExists(metadata::wrappedKek, WRAPPED_KEK, generator);
    JsonUtil.writeStringIfExists(metadata::wrappedDek, WRAPPED_DEK, generator);
    if (null != metadata.algorithm()) {
      generator.writeStringField(ALGORITHM, metadata.algorithm().name());
    }
    JsonUtil.writeBinaryIfExists(metadata::iv, IV, generator);
    JsonUtil.writeBinaryIfExists(metadata::aadTag, AAD_PREFIX, generator);
    JsonUtil.writeStringIfExists(metadata::originalColumnName, ORIGINAL_COLUMN_NAME, generator);
  ...
  public static EnvelopeMetadata fromJson(JsonNode json) throws IOException {
    ...
    String encryptedKek = JsonUtil.getStringOrNull(WRAPPED_KEK, json);
    String encryptedDek = JsonUtil.getStringOrNull(WRAPPED_DEK, json);
    String originalColumnName = JsonUtil.getStringOrNull(ORIGINAL_COLUMN_NAME, json);
    ...
    ByteBuffer aadTag = JsonUtil.getBytesOrNull(AAD_PREFIX, json);
    ...
    EnvelopeMetadata metadata = new EnvelopeMetadata(mekId, kekId, encryptedKek,
            encryptedDek, iv, algorithm, properties, originalColumnName, columnIds, columnMetadata);
