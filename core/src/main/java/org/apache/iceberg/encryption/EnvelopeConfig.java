...
public class EnvelopeConfig implements Serializable {
  ...
  private final boolean doubleEnvelope;
  ...
  public EnvelopeConfig(
    ...
    this.doubleEnvelope = mekId != null;
    if (doubleEnvelope) {
      Preconditions.checkArgument(kekId == null,
              "Cannot construct envelope config because both MEK and KEK ID are specified");
    }
  ...
  public boolean isDoubleEnvelope() {
    return doubleEnvelope;
  }

  /**
   * Properties defined by keys in TODO {link EncryptionProperties}
   * <p>
   * Users can define free-form properties that are used for features such as column data masking.
   * TODO @Jack, looks like we use different definitions of AAD Prefixes. TBD.
   * TODO Properties that starts with {link EncryptionProperties#AAD_PREFIX} is used
   * TODO to form the additional authenticated data (AAD) that will be used during {@link KmsClient#generateKey}
   *
