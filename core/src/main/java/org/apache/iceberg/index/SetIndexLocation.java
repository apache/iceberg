...
class SetIndexLocation implements UpdateLocation {
  private final IndexOperations ops;
  private String newLocation = null;

  SetIndexLocation(IndexOperations ops) {
    this.ops = ops;
  }

  @Override
  public String apply() {
    Preconditions.checkState(null != newLocation, "Invalid index location: null");
    return newLocation;
  }

  @Override
  public void commit() {
    IndexMetadata base = ops.refresh();
    ...
    Tasks.foreach(ops)
        ...
        .run(
            taskOps ->
                taskOps.commit(base, IndexMetadata.buildFrom(base).setLocation(apply()).build()));
  }
