...
/** SPI interface to abstract index metadata access and updates. */
public interface IndexOperations {

  /**
   * Return the currently loaded index metadata, without checking for updates.
   *
  ...
  /**
   * Return the current index metadata after checking for updates.
   *
   * @return index metadata
   */
  IndexMetadata refresh();

  /**
   * Replace the base index metadata with a new version.
   *
   ...
   *
   * @param base index metadata on which changes were based
   * @param metadata new index metadata with updates
   */
  void commit(IndexMetadata base, IndexMetadata metadata);
}
