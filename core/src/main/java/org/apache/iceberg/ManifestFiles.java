...
public class ManifestFiles {
  ...
  /**
   * Returns colocated DV changes for a v4 leaf data manifest as {@code (status, DeleteFile)} pairs:
   * {@link ManifestEntry.Status#ADDED} for newly-live DVs (born-with-DV or DV added/updated) and
   * {@link ManifestEntry.Status#DELETED} for prior DVs that were superseded (preserved on REPLACED
   * rows). Legacy (pre-v4) manifests have no colocated DVs and produce an empty iterable.
   *
  ...
  /**
   * Returns data-file changes from a v4 leaf data manifest as {@code (v4 status, DataFile)} pairs.
   * Unlike {@link #read(ManifestFile, FileIO, Map)}, which collapses REPLACED → DELETED and
   * MODIFIED → EXISTING for legacy consumers, this path surfaces the v4 tracking status directly so
   * callers can distinguish data-file changes (ADDED, DELETED) from DV-state transitions (REPLACED,
   * MODIFIED). Returns an empty iterable for legacy (pre-v4) manifests; callers should dispatch via
   * {@link #isV4ContentEntryManifest(ManifestFile, FileIO)} before invoking.
   */
