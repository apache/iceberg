...
public class HadoopFileIO implements HadoopConfigurable, DelegateFileIO {
   ...
   * Although bulk delete APIs are in the hadoop version iceberg is built with, they are not in the
   * Hadoop 3.3.x releases used by spark 3.x. Dynamically loading them if present avoids problems
   * there.
   */
  ...
  private synchronized boolean maybeUseBulkDeleteApi() {
    ...
    boolean enableBulkDelete = conf().getBoolean(BULK_DELETE_ENABLED, DEFAULT_BULK_DELETE_ENABLED);
    if (!enableBulkDelete) {
  ...
  private int bulkDeleteFiles(Iterable<String> pathnames) {
    ...
    for (String name : pathnames) {
      ...
      // retrieve or create delete path set for the specific filesystem
  ...
  /**
   * Delete a single batch whose size is less than or equal to the page size.
   *
   * @param fs filesystem.
