...
final class BulkDeleter {
   ...
   * page size of 1. On S3 a larger bulk delete operation is supported, with the page size set by
  ...
  public int bulkDeleteFiles(Iterable<String> pathnames) {
    ...
    Map<Path, StoreDeletion> deletionMap = Maps.newHashMap();
    ...
    for (String name : pathnames) {
      ...
      StoreDeletion storeDeletion = deletionMap.get(fsRoot);
      if (storeDeletion == null) {
        // fs root is not in the map, so ask for and then cache
        // the page size that fs supports.
        try {
          storeDeletion = new StoreDeletion(fs.createBulkDelete(fsRoot));
          deletionMap.put(fsRoot, storeDeletion);
        } catch (IOException e) {
          // bulk delete creation failure.
          throw new UncheckedIOException(e);
        }
      }

      // add the deletion target.
      storeDeletion.add(target);

      if (storeDeletion.pageIsComplete()) {
        // the page size has been reached.

        // get the live path list, which MUST be done outside the async
        // submitted closure.
        final Collection<Path> paths = storeDeletion.snapshotDeletedFiles();
        final BulkDelete bulkDeleter = storeDeletion.bulkDeleter();
        // execute the bulk delete in a new thread then prepare the delete path set for new entries.
  ...
  private static final class StoreDeletion {
    private final BulkDelete bulkDeleter;
    private final int pageSize;
    private Set<Path> deletedFiles;

    private StoreDeletion(BulkDelete bulkDeleter) {
      this.bulkDeleter = bulkDeleter;
      this.pageSize = bulkDeleter.pageSize();
    }
