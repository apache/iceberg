...
class RemoveTableMetadata implements ExpireTableMetadata {
  ...
  private Integer expireExcept = null;
  ...
  @Override
  public ExpireTableMetadata expireExcept(int number) {
    LOG.info("Expiring all table metadata files except of last {}", number);
    this.expireExcept = number;
    return this;
  }

  @Override
  public ExpireTableMetadata expireOlderThan(long timestampMillis) {
    LOG.info("Expiring table metadata older than: {} ({})", new Date(timestampMillis), timestampMillis);
  ...
  public List<TableMetadataFile> apply() {
    ...
    return StreamSupport.stream(ops.tableMetadataFiles().spliterator(), false)
        .filter(metadataFile -> currentVersion != metadataFile.version() &&
          ((expireExcept != null && currentVersion - expireExcept >= metadataFile.version()) ||
