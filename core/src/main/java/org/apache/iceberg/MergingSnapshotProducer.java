...
abstract class MergingSnapshotProducer<ThisT> extends SnapshotProducer<ThisT> {
  ...
  private PositionDeleteIndex mergeConflictingDVs(Iterable<DeleteFile> conflictingDVs) {
    ...
    confictingDVIterator.forEachRemaining(dv -> mergedPositions.merge(readDV(dv)));
  ...
  private PositionDeleteIndex readDV(DeleteFile dv) {
    ...
    InputFile inputFile = EncryptingFileIO.combine(ops().io(), ops().encryption()).newInputFile(dv);
