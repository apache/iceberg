...
public class ViewUtil {
  /**
   * Method picks and returns the 'summary' properties from the map of table properties. Summary properties are recorded
   * in the 'summary' portion of 'Version' in metadata json file.
   *
   ...
   * @return A map of summary properties to be recorded in the metadata json file. These are all previously set
   * properties overlaid with the new properties.
   */
  public static Map<String, String> buildSummaryProperties(
    ...
    for (ViewConstants.SummaryConstants key :
        ViewConstants.SummaryConstants.values()) {
  ...
  /**
   * Method picks and returns common view specific properties from the map of table properties. These properties are
   * recorded in the 'properties' section of the view version metadata file. Any properties that were previously set and
   * are not being overridden are persisted.
   *
   * @param properties         Map of all table properties
  ...
  public static void doCommit(
    ...
    // 2) View version metadata properties: these are not versioned. These are all the other table properties
    //                                      that do not belong in 1) or 2) above.
    Map<String, String> summary = ViewUtil.buildSummaryProperties(operation, properties, prevSummaryProps);

    Map<String, String> viewVersionMetadataProperties =
        ViewUtil.getViewVersionMetadataProperties(properties, prevViewVersionMetadataProps, summary);
    // add the owner field to the view version metadata props as it doesn't belong to the other 2 categories.
    ...
    BaseViewVersion version = BaseViewVersion.builder()
        .versionId(versionId)
        .parentId(parentId)
    ...
    ViewMetadata.Builder viewVersionMetadataBuilder = ViewMetadata.builder()
        .location(location)
        .properties(viewVersionMetadataProperties);
    if (prevViewMetadata != null) {
      viewVersionMetadataBuilder.versions(prevViewMetadata.versions())
