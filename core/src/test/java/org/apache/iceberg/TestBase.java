...
public class TestBase {
  ...
  public TestBase(int formatVersion) {
  ...
  void validateSnapshot(Snapshot old, Snapshot snap, Long sequenceNumber, DataFile... newFiles) {
    ...
    for (ManifestFile oldManifest : oldManifests) {
      Assertions.assertThat(newManifests.remove(oldManifest))
    ...
    for (ManifestEntry<DataFile> entry : ManifestFiles.read(manifest, FILE_IO).entries()) {
      ...
      Assertions.assertThat(file.path().toString())
          .as("Path should match expected")
          .isEqualTo(newPaths.next());
      Assertions.assertThat((long) entry.snapshotId())
          .as("File's snapshot ID should match")
          .isEqualTo(id);
    }

    Assertions.assertThat(newPaths.hasNext()).as("Should find all files in the manifest").isFalse();

    Assertions.assertThat((int) snap.schemaId())
        .as("Schema ID should match")
        .isEqualTo(table.schema().schemaId());
  }
  ...
  void validateTableFiles(Table tbl, DataFile... expectedFiles) {
    ...
    Assertions.assertThat(actualFilePaths).as("Files should match").isEqualTo(expectedFilePaths);
  }
  ...
  void validateBranchDeleteFiles(Table tbl, String branch, DeleteFile... expectedFiles) {
    ...
    Assertions.assertThat(actualFilePaths)
        .as("Delete files should match")
  ...
  void validateManifest(
    ...
    for (ManifestEntry<DataFile> entry : ManifestFiles.read(manifest, FILE_IO).entries()) {
      ...
      Assertions.assertThat(entry.snapshotId())
          .as("Snapshot ID should match expected ID")
          .isEqualTo(ids.next());
      if (statuses != null) {
        Assertions.assertThat(entry.status())
            .as("Status should match expected")
  ...
  protected static class TableAssertions {
    ...
    void assertEquals(String context, int expected, int actual) {
      if (enabled) {
        Assertions.assertThat(actual).as(context).isEqualTo(expected);
      }
    }
