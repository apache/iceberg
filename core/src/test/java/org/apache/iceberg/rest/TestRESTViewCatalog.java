...
public class TestRESTViewCatalog extends ViewCatalogTests<RESTCatalog> {
  ...
  public void testPaginationForListViews(int numberOfItems) {
    ...
    Mockito.verify(adapter)
        .execute(match(HTTPMethod.GET, "v1/config"), eq(ConfigResponse.class), any(), any());

    Mockito.verify(adapter, times(numberOfItems))
        .execute(
            match(HTTPMethod.POST, String.format("v1/namespaces/%s/views", namespaceName)),
  ...
  public void viewExistsViaHEADRequest() {
    ...
    Mockito.verify(adapter)
        .execute(
            match(HTTPMethod.GET, "v1/config", Map.of(), Map.of()),
            eq(ConfigResponse.class),
            any(),
            any());
    Mockito.verify(adapter)
        .execute(
            match(HTTPMethod.HEAD, "v1/namespaces/ns/views/view", Map.of(), Map.of()),
            any(),
            any(),
            any());
  }
  ...
  private void verifyViewExistsFallbackToGETRequest(
    ...
    Mockito.verify(adapter)
        .execute(
            match(HTTPMethod.GET, "v1/namespaces/ns/views/view", Map.of(), Map.of()),
            any(),
            any(),
            any());
  }
  ...
  public void testCustomViewOperationsInjection() throws Exception {
    ...
    try (RESTCatalog catalog =
      ...
      Mockito.verify(adapter, Mockito.atLeastOnce())
          .execute(
              match(HTTPMethod.POST, resourcePaths.view(viewIdentifier), customHeaders),
              eq(LoadViewResponse.class),
              any(),
              any());
    }
