...
public class ParameterizedTableTestBase {
  ...
  public static Object[][] parameters() {
    return new Object[][] {
      {1, new TableAssertions(1, 1), new TableAssertions(2, 1)},
      {2, new TableAssertions(1, 2), new TableAssertions(2, 2)}
    };
  }
  ...
  ManifestFile writeManifestWithName(String name, DataFile... files) throws IOException {
    File manifestFile = File.createTempFile(name, ".avro", temp.toFile());
    assertThat(manifestFile.delete()).isTrue();
    OutputFile outputFile = table.ops().io().newOutputFile(manifestFile.getCanonicalPath());
  ...
  void validateSnapshot(Snapshot old, Snapshot snap, Long sequenceNumber, DataFile... newFiles) {
    assertThat(Sets.newHashSet(snap.deleteManifests(FILE_IO)))
        .as("Should not change delete manifests")
        .isEqualTo(old != null ? Sets.newHashSet(old.deleteManifests(FILE_IO)) : ImmutableSet.of());
    ...
    for (ManifestEntry<DataFile> entry : ManifestFiles.read(manifest, FILE_IO).entries()) {
      ...
      assertThat(id).as("File's snapshot ID should match").isEqualTo(entry.snapshotId());
    }

    assertThat(newPaths).as("Should find all files in the manifest").isExhausted();
  ...
  protected static class TableAssertions {
    ...
    protected TableAssertions(int validForVersion, int formatVersion) {
