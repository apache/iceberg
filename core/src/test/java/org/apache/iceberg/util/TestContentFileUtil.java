...
public class TestContentFileUtil {
  ...
  @Test
  public void testDVDesc() {
    DeleteFile dv = mockDV("/path/to/dv.puffin", 100L, 50L, "/path/to/data/file.parquet");
    String desc = ContentFileUtil.dvDesc(dv);
    assertThat(desc)
        .contains("location=/path/to/dv.puffin")
        .contains("offset=100")
        .contains("length=50")
        .contains("referencedDataFile=/path/to/data/file.parquet");
  }
