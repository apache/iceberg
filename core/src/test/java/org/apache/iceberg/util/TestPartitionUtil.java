...
public class TestPartitionUtil {
  ...
  void testConvertPartitionFuncTruncateInteger() {
    ...
    StructLike result = converter.apply(sourcePartition);
    // XXX: truncate integer is not supported as only TruncateString defined satisfiesOrderOf()
    assertThat(result.get(0, Integer.class)).isNull();
  }
  ...
  void testConvertPartitionFuncMultipleFields() {
    // Test converting with multiple partition fields
    PartitionSpec sourceSpec =
  ...
  void testConvertPartitionFuncSubsetOfFields() {
    // Test when output spec has subset of source spec fields
    PartitionSpec sourceSpec =
