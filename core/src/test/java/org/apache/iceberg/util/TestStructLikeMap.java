...
public class TestStructLikeMap {
  ...
  public void testNullKeys() {
    Map<StructLike, String> newMap = StructLikeMap.create(KEY_TYPE);
    Assert.assertFalse(newMap.containsKey(null));

    newMap.put(null, "aaa");
    Assert.assertTrue(newMap.containsKey(null));
    Assert.assertEquals("aaa", newMap.get(null));

    String replacedValue = newMap.put(null, "bbb");
    Assert.assertEquals("aaa", replacedValue);

    String removedValue = newMap.remove(null);
  ...
  public void testKeysWithNulls() {
    ...
    Map<StructLike, String> newMap = StructLikeMap.create(KEY_TYPE);
    newMap.put(record1, "aaa");
    newMap.put(record2, "bbb");

    Assert.assertEquals("aaa", newMap.get(record1));
    Assert.assertEquals("bbb", newMap.get(record2));

    Record record3 = record1.copy();
    Assert.assertTrue(newMap.containsKey(record3));
    Assert.assertEquals("aaa", newMap.get(record3));

    Assert.assertEquals("aaa", newMap.remove(record3));
  }
}
