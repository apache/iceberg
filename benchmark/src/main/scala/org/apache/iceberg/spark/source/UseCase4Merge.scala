...
/**
 * A MERGE job that generates 25 updates and 25 inserts in 10% of partitions without
 * any static partition predicates, highligting the importance of runtime filtering.
 */
object UseCase4Merge extends UseCaseBase {

  private final val LOG = LogManager.getLogger(UseCase4Merge.getClass)
  ...
  private final val SOURCE_TABLE_NAME = s"store_sales_use_case_4_${MODE_SHORT}_changes"
  ...
  private final val MERGE_RESULTS = s"use_case_4_${MODE_SHORT}_merge_results"
  private final val QUERY_RESULTS = s"use_case_4_${MODE_SHORT}_query_results"
  private final val WARMUP_RESULTS = s"use_case_4_${MODE_SHORT}_warmup_results"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder()
      .appName(s"vldb-use-case-4-${MODE_SHORT}")
      .getOrCreate()
