...
/**
 * An upsert job that generates 1% of updates and 25 inserts in each partition.
 */
object UseCase2Upsert extends UseCaseBase {

  private final val LOG = LogManager.getLogger(UseCase2Upsert.getClass)
  private final val TARGET_TABLE_STARTING_SNAPSHOT = 5074150813535168321L
  private final val MODE_SHORT = "upsert"
  private final val SOURCE_TABLE_NAME = s"store_sales_use_case_2_${MODE_SHORT}_changes"
  private final val NUM_BUCKETS = 256
  private final val NUM_UPDATES_PER_PARTITION = 110000
  private final val NUM_INSERTS_PER_PARTITION = 25
  private final val NUM_OPERATIONS = 10
  private final val MERGE_RESULTS = s"use_case_2_${MODE_SHORT}_merge_results"
  private final val QUERY_RESULTS = s"use_case_2_${MODE_SHORT}_query_results"
  private final val WARMUP_RESULTS = s"use_case_2_${MODE_SHORT}_warmup_results"

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder()
      .appName(s"vldb-use-case-2-${MODE_SHORT}")
      .getOrCreate()
    ...
    for (operation <- 0 until NUM_OPERATIONS) {
      ...
      val (_, mStart, mEnd, mDuration) = time {
        UpsertUtil.applyChanges(
          spark,
          targetTableIdent = s"$CATALOG.$TARGET_TABLE_NAME",
          sourceTableIdent = s"$CATALOG.$SOURCE_TABLE_NAME",
          eqCols = Seq("ss_ticket_number", "ss_item_sk"))
      }
      mResults += ((operation, mStart, mEnd, mDuration, mDuration / 1000.0 / 1000.0 / 1000.0))

      spark.sql(s"REFRESH TABLE $CATALOG.$TARGET_TABLE_NAME")
