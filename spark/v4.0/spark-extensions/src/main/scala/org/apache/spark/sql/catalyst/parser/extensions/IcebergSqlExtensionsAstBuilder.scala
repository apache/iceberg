...
class IcebergSqlExtensionsAstBuilder(delegate: ParserInterface) extends IcebergSqlExtensionsBaseVisitor[AnyRef] {
  ...
  override def visitSetWriteDistributionAndOrdering(
    ...
    if (distributionSpec == null && orderingSpec == null) {
      throw new AnalysisException(
        errorClass ="ALTER TABLE has no changes: missing both distribution and ordering clauses", messageParameters = Map.empty)
    }
  ...
  private def toDistributionAndOrderingSpec(
    ...
    if (writeSpec.writeDistributionSpec.size > 1) {
      throw new AnalysisException(errorClass ="ALTER TABLE contains multiple distribution clauses", messageParameters = Map.empty)
    }

    if (writeSpec.writeOrderingSpec.size > 1) {
      throw new AnalysisException(errorClass ="ALTER TABLE contains multiple ordering clauses", messageParameters = Map.empty)
    }
