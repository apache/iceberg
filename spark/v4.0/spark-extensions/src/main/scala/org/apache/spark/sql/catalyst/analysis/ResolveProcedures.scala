...
case class ResolveProcedures(spark: SparkSession) extends Rule[LogicalPlan] with LookupCatalog {
  ...
  private def validateParams(params: Seq[ProcedureParameter]): Unit = {
    ...
    if (duplicateParamNames.nonEmpty) {
      throw new AnalysisException(errorClass = s"Duplicate parameter names: ${duplicateParamNames.mkString("[", ",", "]")}", messageParameters = Map.empty)
    }
    ...
    params.sliding(2).foreach {
      case Seq(previousParam, currentParam) if !previousParam.required && currentParam.required =>
        throw new AnalysisException(
          errorClass =  s"Optional parameters must be after required ones but $currentParam is after $previousParam", messageParameters = Map.empty)
      case _ =>
    }
  }
  ...
  private def buildArgExprs(
    ...
    if (missingParamNames.nonEmpty) {
      throw new AnalysisException(errorClass =s"Missing required parameters: ${missingParamNames.mkString("[", ",", "]")}", messageParameters = Map.empty)
    }
  ...
  private def buildNameToArgMap(
    ...
    if (containsNamedArg && containsPositionalArg) {
      throw new AnalysisException(errorClass ="Named and positional arguments cannot be mixed", messageParameters = Map.empty)
    }
  ...
  private def buildNameToArgMapUsingNames(
    ...
    if (validationErrors.nonEmpty) {
      throw new AnalysisException(errorClass =s"Could not build name to arg map: ${validationErrors.mkString(", ")}", messageParameters = Map.empty)
    }
  ...
  private def buildNameToArgMapUsingPositions(
    ...
    if (args.size > params.size) {
      throw new AnalysisException(errorClass ="Too many arguments for procedure", messageParameters = Map.empty)
    }
  ...
  implicit class CatalogHelper(plugin: CatalogPlugin) {
    def asProcedureCatalog: ProcedureCatalog = plugin match {
      case procedureCatalog: ProcedureCatalog =>
        procedureCatalog
      case _ =>
        throw new AnalysisException(errorClass =s"Cannot use catalog ${plugin.name}: not a ProcedureCatalog", messageParameters = Map.empty)
    }
  }
}
