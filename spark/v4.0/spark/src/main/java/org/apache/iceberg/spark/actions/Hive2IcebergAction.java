...
/**
 * This operation converts a regular Hive table to an Iceberg table. The steps are as follows:
 *
 * <p>Step 1: Create a new snapshot table for the source table (using {@link
 * org.apache.iceberg.spark.actions.SnapshotTableSparkAction}), with a table name in the format
 * tableName_snapshot_yyyyMMdd_HHmmss. Step 2: Use {@link
 * org.apache.iceberg.spark.actions.RewriteTablePathSparkAction} to copy the metadata from the
 * snapshot table to the source table. Step 3: Call the Hive Metastore (HMS) interface to add
 * necessary properties (e.g., metadata_location, table_type, provider), converting the regular Hive
 * table to an Iceberg table. Step 4: Delete the created snapshot table.
 *
 * <p>After these steps, the regular Hive table will be successfully converted to an Iceberg table.
 */
public class Hive2IcebergAction extends BaseTableCreationSparkAction<Hive2IcebergAction>
  ...
  Hive2IcebergAction(
      SparkSession pSpark, CatalogPlugin pSourceCatalog, Identifier pSourceTableIdent) {
    super(pSpark, pSourceCatalog, pSourceTableIdent);
    this.sourceTableIdent = pSourceTableIdent;
    this.sparkSession = pSpark;
    this.sourceCatalog = pSourceCatalog;
    initializeDestCatalogAndIdentifier(pSourceTableIdent.name());
  }
  ...
  public Result execute() {
    String desc = String.format("Hive2Iceberg table %s.", sourceTableIdent());
    JobGroupInfo info = newJobGroupInfo("HIVE-TO-ICEBERG-TABLE", desc);
    return withJobGroupInfo(info, this::doExecute);
  }

  private Hive2Iceberg.Result doExecute() {
    ...
    if (!snapshotSuccess) {
      return buildFailureResult("hive2iceberg snapshot creation failed");
    }

    // Step2. Rewrite Table Path
    // Prepare SourceTable
    OperationSourceTableResult sourceTableResult = handleSourceTablePreparation();
    if (!sourceTableResult.isSuccess()) {
      return buildFailureResult("hive2iceberg source table preparation failed");
    }
    ...
    if (!targetTableResult.isSuccess()) {
      return buildFailureResult("hive2iceberg target table preparation failed");
    }
    ...
    if (!handlePathRewrite.isSuccess()) {
      return buildFailureResult("hive2iceberg rewrite table path failed");
    }

    String latestVersion = handlePathRewrite.getLatestVersion();
    String targetPrefix =
        Optional.ofNullable(sourceSparkTable.storage().locationUri().get())
            .map(CatalogUtils::URIToString)
            .orElseThrow(
                () -> new IllegalStateException("Source table location URI is not available"));

    // Step3. MetadataMigration
    boolean metadataMigration =
        handleMetadataMigration(targetPrefix, latestVersion, sourceIdentifier);
    if (!metadataMigration) {
      return buildFailureResult("hive2iceberg metadata migration failed");
    }

    // Step4. Clear SnapshotTable
    boolean cleanSnapShotTable = handleClearSnapShotTable(snapShotTableName, snapShotSparkTable);
    if (!cleanSnapShotTable) {
      return buildFailureResult("hive2iceberg clean snapshot table failed");
    }
  ...
  private boolean handleSnapshotCreation(String snapShotTableName) {
    try {
      ...
      LOG.error("hive2iceberg snapshot creation failed", e);
      return false;
    }
  }
    ...
    try {
      ...
      Spark3Util.CatalogAndIdentifier catalogAndIdent =
          Spark3Util.catalogAndIdentifier(
              ctx, sparkSession, sourceTableIdent.name(), catalogPlugin);
  ...
  private OperationTargetTableResult handleTargetTablePreparation(String targetTableName) {
    ...
    try {
      ...
      Spark3Util.CatalogAndIdentifier catalogAndIdent = getCatalogAndIdent(ctx, targetTableName);
      TableCatalog tableCatalog = checkTargetCatalog(catalogAndIdent.catalog());
  ...
  private OperationPathRewriteResult handlePathRewrite(
    ...
    try {
      ...
      rewriteAction.createFileList(false);
      ...
      LOG.error("hive2iceberg rewrite table path failed", e);
  ...
  private boolean handleMetadataMigration(
    ...
    try {
      ...
      hiveCatalog.setConf(spark().sessionState().newHadoopConf());
      ...
      targetSourceCatalog.alterTable(
          ...
          TableChange.setProperty("provider", "iceberg"));
      return true;
    } catch (Exception e) {
      LOG.error("hive2iceberg metadata migration failed", e);
      return false;
    }
  }
  ...
  private boolean handleClearSnapShotTable(String snapShotTable, SparkTable snapShotSparkTable) {
    try {
      String ctx = "hive2iceberg clean snapshot table";
      Spark3Util.CatalogAndIdentifier catalogAndIdent = getCatalogAndIdent(ctx, snapShotTable);
      ...
      LOG.info("hive2iceberg clean snapshot table: {}.", snapShotSparkTable.name());
      return true;
    } catch (Exception e) {
      LOG.error("hive2iceberg clean snapshot table failed", e);
      return false;
    }
  }
  ...
  private void initializeDestCatalogAndIdentifier(String ident) {
    String ctx = "hive2iceberg destination";
    Spark3Util.CatalogAndIdentifier catalogAndIdent = getCatalogAndIdent(ctx, ident);
    this.destCatalog = checkDestinationCatalog(catalogAndIdent.catalog());
    this.destTableIdent = catalogAndIdent.identifier();
  }

  private Spark3Util.CatalogAndIdentifier getCatalogAndIdent(String ctx, String ident) {
    CatalogPlugin defaultCatalog = spark().sessionState().catalogManager().currentCatalog();
    return Spark3Util.catalogAndIdentifier(ctx, spark(), ident, defaultCatalog);
  }
