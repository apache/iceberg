...
public class IcebergHiveConnectorDelegationTokenProvider implements HadoopDelegationTokenProvider {
  ...
  public Option<Object> obtainDelegationTokens(
    ...
    Map<String, IMetaStoreClient> hmsClientCache = new HashMap<>();
    ...
    try {
      for (String catalogName : requireTokenCatalogs) {
        ...
        if (!hiveConfOpt.isPresent()) continue;
            ...
            client -> {
              try {
                ...
                throw new RuntimeException(
                    "Failed to close HiveMetaStoreClient: " + e.getMessage(), e);
              }
            });
  }

  private <T> void doAsRealUser(PrivilegedExceptionAction<T> action) throws Throwable {
    UserGroupInformation currentUser = UserGroupInformation.getCurrentUser();
    ...
    try {
      ...
      if (e.getCause() instanceof Exception) {
        ...
        LOG.error("Fatal error int doAsRealUser: {}", e.getMessage(), e);
        throw new Exception("Fatal error in doAsRealUser: " + e.getCause());
      }
    }
  }
}
