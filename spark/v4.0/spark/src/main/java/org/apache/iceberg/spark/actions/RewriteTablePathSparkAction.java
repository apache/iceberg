...
public class RewriteTablePathSparkAction extends BaseSparkAction<RewriteTablePath>
  ...
  private ForeachFunction<DeleteFile> rewritePositionDelete(
    ...
    Map<String, String> mappings = this.prefixMappings;
    return deleteFile -> {
      FileIO io = tableArg.getValue().io();
      String newPath =
          RewriteTablePathUtil.stagingPath(deleteFile.location(), mappings, stagingLocationArg);
      ...
      RewriteTablePathUtil.rewritePositionDeleteFile(
          deleteFile, outputFile, io, spec, mappings, posDeleteReaderWriter);
    };
  }
