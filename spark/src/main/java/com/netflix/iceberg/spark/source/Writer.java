...
class Writer implements DataSourceWriter {
  ...
  public DataWriterFactory<InternalRow> createWriterFactory() {
    return new WriterFactory(table.spec(), format, table.properties(), fileIo);
  }
  ...
  private static class WriterFactory implements DataWriterFactory<InternalRow> {
    ...
    public DataWriter<InternalRow> createDataWriter(int partitionId, long taskId, long epochId) {
      ...
      if (spec.fields().isEmpty()) {
        return new UnpartitionedWriter(filename, format, factory, fileIo);
      } else {
        return new PartitionedWriter(filename, spec, format, factory, fileIo);
      }
    }
  ...
  private static class PartitionedWriter implements DataWriter<InternalRow> {
    ...
    public void abort() throws IOException {
      ...
      if (currentAppender != null) {
        ...
        fileIo.deleteFile(currentFile.location());
      }
    }
    ...
    private void closeCurrent() throws IOException {
      if (currentAppender != null) {
        ...
        InputFile inFile = fileIo.newInputFile(currentFile.location());
