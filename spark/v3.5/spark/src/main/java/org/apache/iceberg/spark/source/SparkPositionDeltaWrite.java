...
class SparkPositionDeltaWrite implements DeltaWrite, RequiresDistributionAndOrdering {
  ...
  private class PositionDeltaBatchWrite implements DeltaBatchWrite {
    ...
    public DeltaWriterFactory createBatchWriterFactory(PhysicalWriteInfo info) {
      ...
      Broadcast<Map<String, DeleteFileSet>> dataFileToFileScopedDeletesBroadcast =
          sparkContext.broadcast(
              scan != null && context.maintainDeletesDuringWrite()
                  ? scan.dataToFileScopedDeletes()
                  : Maps.newHashMap());
      return new PositionDeltaWriteFactory(
          tableBroadcast, dataFileToFileScopedDeletesBroadcast, command, context, writeProperties);
    }
  ...
  private static class PositionDeltaWriteFactory implements DeltaWriterFactory {
    ...
    private final Broadcast<Map<String, DeleteFileSet>> dataFileToFileScopedDeletesBroadcast;
    private final Command command;
    private final Context context;
    private final Map<String, String> writeProperties;
    ...
    PositionDeltaWriteFactory(
      ...
      this.dataFileToFileScopedDeletesBroadcast = dataFileToFileScopedDeletesBroadcast;
    }

    @Override
    public DeltaWriter<InternalRow> createWriter(int partitionId, long taskId) {
      Table table = tableBroadcast.value();
      Map<String, DeleteFileSet> dataFileToFileScopedDeletes =
          dataFileToFileScopedDeletesBroadcast.getValue();
      ...
      if (command == DELETE) {
        return new DeleteOnlyDeltaWriter(
            table, dataFileToFileScopedDeletes, writerFactory, deleteFileFactory, context);

      } else if (table.spec().isUnpartitioned()) {
        return new UnpartitionedDeltaWriter(
            table,
            dataFileToFileScopedDeletes,
            writerFactory,
            dataFileFactory,
            deleteFileFactory,
            context);
  ...
  private abstract static class BaseDeltaWriter implements DeltaWriter<InternalRow> {
    ...
    protected PartitioningWriter<PositionDelete<InternalRow>, DeleteWriteResult> newDeleteWriter(
        Table table,
        Map<String, DeleteFileSet> dataFileToDeleteFiles,
      ...
      if (inputOrdered) {
        ...
        return new FanoutPositionOnlyDeleteWriter<>(
            ...
            targetFileSize,
            deleteGranularity,
            new PreviousDeleteLoader(table, dataFileToDeleteFiles));
      }
    }
  }
  ...
  private static class Context implements Serializable {
    ...
    private final boolean maintainDeletesDuringWrite;
    ...
    Context(
      ...
      this.maintainDeletesDuringWrite = writeConf.maintainPositionDeletesDuringWrite();
    }
    ...
    boolean maintainDeletesDuringWrite() {
      return maintainDeletesDuringWrite;
    }
  }
}
