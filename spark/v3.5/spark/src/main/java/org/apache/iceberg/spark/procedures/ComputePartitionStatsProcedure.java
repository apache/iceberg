...
/**
 * A procedure that computes, writes and registers partition statistics of a table.
 *
...
public class ComputePartitionStatsProcedure extends BaseProcedure {
  ...
  private static final ProcedureParameter BRANCH =
      ProcedureParameter.optional("branch", DataTypes.StringType);

  private static final ProcedureParameter[] PARAMETERS =
      new ProcedureParameter[] {TABLE_PARAM, BRANCH};
  ...
  public InternalRow[] call(InternalRow args) {
    ...
    String branch = input.asString(BRANCH, null);

    return modifyIcebergTable(
        tableIdent,
        table -> {
          ComputePartitionStats action = actions().computePartitionStats(table).branch(branch);
