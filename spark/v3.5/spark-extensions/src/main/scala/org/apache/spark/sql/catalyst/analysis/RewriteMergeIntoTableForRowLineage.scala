...
object RewriteMergeIntoTableForRowLineage extends RewriteOperationForRowLineage {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    RewriteMergeIntoTable.apply(plan) resolveOperators {
      ...
      case append@AppendData(table, _, _, _,  _, _) if updatePlanForRowLineage(append) =>
        val output = append.query.output

        val outputExprs = output ++ Seq(Literal(null), Literal(null))
        val outputColNames = table.output.map(_.name) ++ Seq("_row_id", "_last_updated_sequence_number")
        val outputCols = outputExprs.zip(outputColNames).map { case (expr, name) =>
          Alias(expr, name)()
        }

        val updatedProjection = Project(outputCols, append.child)
        val lineageColumns: Seq[AttributeReference] = table.metadataOutput.filter(attr => attr.name == "_row_id"
          || attr.name == "_last_updated_sequence_number").map(_.asInstanceOf[AttributeReference])
        append.withNewTable(
          append.table.asInstanceOf[DataSourceV2Relation]
            .copy(output = append.table.output.map(_.asInstanceOf[AttributeReference]) ++ lineageColumns))
        .withNewQuery(updatedProjection)
