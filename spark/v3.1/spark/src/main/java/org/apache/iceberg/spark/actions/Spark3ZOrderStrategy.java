...
public class Spark3ZOrderStrategy extends Spark3SortStrategy {
  private static final Logger LOG = LoggerFactory.getLogger(Spark3ZOrderStrategy.class);
  ...
  public Set<DataFile> rewriteFiles(List<FileScanTask> filesToRewrite) {
    Spark3ZOrderUDF zOrderUDF =
        new Spark3ZOrderUDF(zOrderColNames.size(), varLengthContribution, maxOutputSize);
    ...
    if (requiresRepartition) {
      ordering = Spark3Util.convert(SortOrderUtil.buildSortOrder(table(), sortOrder()));
    } else {
