 ...
 */
case object RewriteStaticInvoke extends Rule[LogicalPlan] with SQLConfHelper {
  ...
  private def replaceStaticInvoke(expression: Expression): Expression = {
    expression.transform {
      case invoke: StaticInvoke =>
        invoke.functionName match {
          case "invoke" =>
            toDataTypes(invoke.inputTypes) match {
              case Some(inputDataTypes) =>
                val systemFunc = SparkFunctions.buildFromClass(invoke.staticObject, inputDataTypes)
                if (systemFunc == null) {
                  invoke
                } else {
                  ApplyFunctionExpression(systemFunc, invoke.arguments)
                }
