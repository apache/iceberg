...
public class RewriteTablePathSparkAction extends BaseSparkAction<RewriteTablePath>
  ...
  private Set<Pair<String, String>> statsFileCopyPlan(
    ...
    for (int i = 0; i < beforeStats.size(); i++) {
      ...
      result.add(
          Pair.of(RewriteTablePathUtil.stagingPath(before.path(), sourcePrefix, stagingDir), after.path()));
    }
    return result;
  }
  ...
  private static RewriteResult<DataFile> writeDataManifest(
      ...
      String targetPrefix) {
    try {
      String stagingPath = RewriteTablePathUtil.stagingPath(manifestFile.path(), sourcePrefix, stagingLocation);
  ...
  private ForeachFunction<DeleteFile> rewritePositionDelete(
    ...
    return deleteFile -> {
      ...
      String newPath = RewriteTablePathUtil.stagingPath(deleteFile.location(), sourcePrefixArg, stagingLocationArg);
