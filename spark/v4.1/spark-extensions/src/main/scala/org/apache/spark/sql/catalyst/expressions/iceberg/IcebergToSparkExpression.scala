 ...
 *
 * The wire format references columns by field id; ids are resolved to names against the table
 * schema when the response is parsed
 * (`org.apache.iceberg.rest.restrictions.ReadRestrictionsParser`), so by the time a filter reaches
 * this converter its references are already names.
 *
 * Conversion covers the subset defined by the REST OpenAPI `Predicate` schema: True/False,
  ...
  private def toCatalyst(expr: Expression, byName: Map[String, Attribute]): CatalystExpression = {
    expr match {
      ...
      case pred: UnboundPredicate[_] =>
        // pred.ref returns the reference underneath a transform or function application, so a
        // predicate over apply(f, col) would otherwise be converted as if it were over col --
        // silently dropping f and widening the rows the filter admits. Fail closed instead.
        if (!pred.term.isInstanceOf[NamedReference[_]]) {
          throw new UnsupportedOperationException(
            s"Cannot convert row filter predicate over a non-reference term: ${pred.term}")
        }
