...
class SparkWriteBuilder implements WriteBuilder, SupportsDynamicOverwrite, SupportsOverwrite {
  ...
  private Mode mode() {
    return mode != null ? mode : new Append();
  }
  ...
  public WriteBuilder overwrite(Filter[] filters) {
    ...
    String overwriteMode = writeConf.overwriteMode();
    if (expr == Expressions.alwaysTrue() && "dynamic".equals(overwriteMode)) {
      // use the write option to override truncating the table. use dynamic overwrite instead.
      this.mode = new DynamicOverwrite();
    } else {
      this.mode = new OverwriteByFilter(expr);
    }
    return this;
  }
  ...
  private StructType sparkWriteSchema() {
    if (writeIncludesRowLineage()) {
      ...
      schema = schema.add(rowId);
      StructField seq = metaSchema.apply(MetadataColumns.LAST_UPDATED_SEQUENCE_NUMBER.name());
      schema = schema.add(seq);
      return schema;
    } else {
      return info.schema();
    }
  ...
  public Write build() {
    ...
    return new SparkWrite(
      ...
      public BatchWrite toBatch() {
        ...
        if (currentMode instanceof OverwriteByFilter overwrite) {
          return asOverwriteByFilter(overwrite.expr());
        } else if (currentMode instanceof DynamicOverwrite) {
          return asDynamicOverwrite();
        } else if (currentMode instanceof CopyOnWriteOperation cow) {
      ...
      public StreamingWrite toStreaming() {
        Mode currentMode = mode();
        if (currentMode instanceof Append) {
          return asStreamingAppend();
        } else if (currentMode instanceof OverwriteByFilter overwrite) {
          ...
          throw new IllegalStateException("Unsupported streaming write mode: " + currentMode);
        }
      }
    };
  }
  ...
  private void validateRowLineage() {
    Preconditions.checkArgument(
        ...
        "Row lineage information is missing for write in %s mode",
        mode());
  }
  ...
  // merge schema flow:
  // - convert the Spark schema and assign fresh IDs for new fields
  // - update the table to get final ID assignments and validate the changes
  // - reconvert the Spark schema without assignment to use the IDs assigned by UpdateSchema
  ...
  sealed interface Mode {
    ...
    // copy-on-write operation (UPDATE/DELETE/MERGE) that rewrites affected files
