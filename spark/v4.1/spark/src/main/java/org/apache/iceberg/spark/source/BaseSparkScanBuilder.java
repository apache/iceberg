...
abstract class BaseSparkScanBuilder
  ...
  public void pruneColumns(StructType requestedType) {
    ...
    StructType requestedSchema = new StructType(dataFields.toArray(new StructField[0]));

    // the projection should include all columns that will be returned,
    // including those only used in filters
    this.projection = SparkSchemaUtil.prune(projection, requestedSchema, filter(), caseSensitive);
  }
  ...
  private Set<Integer> allUsedFieldIds() {
    return table.schemas().values().stream()
        .flatMap(schema -> TypeUtil.getProjectedIds(schema.asStruct()).stream())
