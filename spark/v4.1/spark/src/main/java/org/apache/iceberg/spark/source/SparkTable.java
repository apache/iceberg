...
public class SparkTable extends BaseSparkTable
  ...
  public ScanBuilder newScanBuilder(CaseInsensitiveStringMap options) {
    if (Spark3Util.containsIncrementalOptions(options)) {
      ...
      return new SparkIncrementalScanBuilder(spark(), table(), options);
    } else {
    ...
    try (CloseableIterable<FileScanTask> tasks = scan.planFiles()) {
      ...
      return Iterables.all(
          ...
          task -> {
            ...
            Evaluator evaluator = getOrCreateEvaluator(deleteExpr, spec, evaluators);
  ...
  private Evaluator getOrCreateEvaluator(
      Expression expr, PartitionSpec spec, Map<Integer, Evaluator> evaluators) {
    return evaluators.computeIfAbsent(spec.specId(), specId -> newEvaluator(spec, expr));
  }

  private Evaluator newEvaluator(PartitionSpec spec, Expression expr) {
    Expression projectedExpr = Projections.strict(spec).project(expr);
    return new Evaluator(spec.partitionType(), projectedExpr);
  }
