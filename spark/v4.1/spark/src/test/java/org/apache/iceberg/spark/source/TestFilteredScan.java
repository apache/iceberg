...
public class TestFilteredScan {
  ...
  public void limitPushedDownToSparkScan() {
    ...
    // verify CoW scan
    AbstractObjectAssert<?, ?> cowScanAssert =
        assertThat(builder.buildCopyOnWriteScan()).extracting("scan");
    if (LOCAL == planningMode) {
      cowScanAssert = cowScanAssert.extracting("scan");
    }
    cowScanAssert.extracting("context").extracting("minRowsRequested").isEqualTo(limit);
