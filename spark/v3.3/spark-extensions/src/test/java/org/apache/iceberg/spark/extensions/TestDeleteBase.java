...
abstract class TestDeleteBase extends SparkRowLevelOperationsTestBase {

  protected TestDeleteBase(
      String catalogName,
  ...
  protected void testDeleteWithoutScanningTable(String branch) throws Exception {
    createAndInitPartitionedTable();

    append(branch, new Employee(1, "hr"), new Employee(3, "hr"));
    append(branch, new Employee(1, "hardware"), new Employee(2, "hardware"));
    ...
    withUnavailableLocations(
        ...
        () -> {
          LogicalPlan parsed =
              parsePlan("DELETE FROM %s WHERE dep = 'hr'", tableNameWithBranch(branch));
  ...
  protected void testDeleteFileThenMetadataDelete(String branch) throws Exception {
    Assume.assumeFalse("Avro does not support metadata delete", fileFormat.equals("avro"));
    createAndInitUnpartitionedTable();

    sql(
        "INSERT INTO TABLE %s VALUES (1, 'hr'), (2, 'hardware'), (null, 'hr')",
        tableNameWithBranch(branch));

    // MOR mode: writes a delete file as null cannot be deleted by metadata
    sql("DELETE FROM %s AS t WHERE t.id IS NULL", tableNameWithBranch(branch));

    // Metadata Delete
    Table table = Spark3Util.loadIcebergTable(spark, tableNameWithBranch(branch));
    Set<DataFile> dataFilesBefore = TestHelpers.dataFiles(table, branch);

    sql("DELETE FROM %s AS t WHERE t.id = 1", tableNameWithBranch(branch));
  ...
  protected void testDeleteWithFalseCondition(String branch) {
    createAndInitUnpartitionedTable();

    sql("INSERT INTO TABLE %s VALUES (1, 'hr'), (2, 'hardware')", tableNameWithBranch(branch));

    sql("DELETE FROM %s WHERE id = 1 AND id > 20", tableNameWithBranch(branch));
  ...
  protected void testDeleteFromEmptyTable(String branch) {
    createAndInitUnpartitionedTable();
  ...
  protected void testExplain(String branch) {
    createAndInitUnpartitionedTable();
  ...
  protected void testDeleteWithAlias(String branch) {
    createAndInitUnpartitionedTable();
  ...
  protected void testDeleteWithDynamicFileFiltering(String branch) throws NoSuchTableException {
    createAndInitPartitionedTable();

    append(branch, new Employee(1, "hr"), new Employee(3, "hr"));
    append(branch, new Employee(1, "hardware"), new Employee(2, "hardware"));

    sql("DELETE FROM %s WHERE id = 2", tableNameWithBranch(branch));
  ...
  protected void testDeleteNonExistingRecords(String branch) {
    createAndInitPartitionedTable();
    ...
    sql("DELETE FROM %s AS t WHERE t.id > 10", tableNameWithBranch(branch));
  ...
  protected void testDeleteWithoutCondition(String branch) {
    createAndInitPartitionedTable();
    ...
    sql("DELETE FROM %s", tableNameWithBranch(branch));
  ...
  protected void testDeleteUsingMetadataWithComplexCondition(String branch) {
    createAndInitPartitionedTable();

    sql("INSERT INTO TABLE %s VALUES (1, 'dep1')", tableNameWithBranch(branch));
  ...
  protected void testDeleteWithArbitraryPartitionPredicates(String branch) {
    createAndInitPartitionedTable();
    ...
    sql("DELETE FROM %s WHERE id = 10 OR dep LIKE '%%ware'", tableNameWithBranch(branch));
  ...
  protected void testDeleteWithNonDeterministicCondition(String branch) {
    createAndInitPartitionedTable();
  ...
  protected void testDeleteWithFoldableConditions(String branch) {
    createAndInitPartitionedTable();
    ...
    // should keep all rows and don't trigger execution
    sql("DELETE FROM %s WHERE 50 <> 50", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(row(1, "hr"), row(2, "hardware")),
        sql("SELECT * FROM %s ORDER BY id", tableNameWithBranch(branch)));
    ...
    sql("DELETE FROM %s WHERE 21 = 21", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(),
        sql("SELECT * FROM %s ORDER BY id", tableNameWithBranch(branch)));
  ...
  protected void testDeleteWithNullConditions(String branch) {
    createAndInitPartitionedTable();
    ...
    sql("DELETE FROM %s WHERE dep = null", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
    ...
    sql("DELETE FROM %s WHERE dep = 'software'", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(row(0, null), row(1, "hr"), row(2, "hardware"), row(null, "hr")),
        sql("SELECT * FROM %s ORDER BY id ASC NULLS LAST", tableNameWithBranch(branch)));
  ...
  protected void testDeleteWithInAndNotInConditions(String branch) {
    createAndInitUnpartitionedTable();
    ...
    sql("DELETE FROM %s WHERE id NOT IN (null, 1)", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(row(2, "hardware"), row(null, "hr")),
        sql("SELECT * FROM %s ORDER BY id ASC NULLS LAST", tableNameWithBranch(branch)));

    sql("DELETE FROM %s WHERE id NOT IN (1, 10)", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
  ...
  protected void testDeleteWithMultipleRowGroupsParquet(String branch) throws NoSuchTableException {
    ...
    sql("DELETE FROM %s WHERE id IN (200, 201)", tableNameWithBranch(branch));

    Assert.assertEquals(199, spark.table(tableNameWithBranch(branch)).count());
  }

  protected void testDeleteWithConditionOnNestedColumn(String branch) {
    ...
    assertEquals(
        ...
        sql("SELECT id FROM %s", tableNameWithBranch(branch)));
  ...
  protected void testDeleteWithInSubquery(String branch) throws NoSuchTableException {
    createAndInitUnpartitionedTable();
    ...
    append(branch, new Employee(1, "hr"), new Employee(-1, "hr"));
    assertEquals(
        "Should have expected rows",
    ...
    append(branch, new Employee(null, "hr"), new Employee(2, "hr"));
    assertEquals(
        "Should have expected rows",
  ...
  protected void testDeleteWithMultiColumnInSubquery(String branch) throws NoSuchTableException {
    createAndInitUnpartitionedTable();

    append(branch, new Employee(1, "hr"), new Employee(2, "hardware"), new Employee(null, "hr"));
    ...
    sql(
        "DELETE FROM %s WHERE (id, dep) IN (SELECT id, dep FROM deleted_employee)",
        tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(row(2, "hardware"), row(null, "hr")),
  ...
  protected void testDeleteWithNotInSubquery(String branch) throws NoSuchTableException {
    createAndInitUnpartitionedTable();
    ...
    sql("DELETE FROM %s WHERE id NOT IN (SELECT * FROM deleted_id)", tableNameWithBranch(branch));
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(row(1, "hr"), row(2, "hardware"), row(null, "hr")),
        sql("SELECT * FROM %s ORDER BY id ASC NULLS LAST", tableNameWithBranch(branch)));
  ...
  protected void testDeleteOnNonIcebergTableNotSupported(String branch) {
  ...
  protected void testDeleteWithExistSubquery(String branch) throws NoSuchTableException {
    createAndInitUnpartitionedTable();
  ...
  protected void testDeleteWithNotExistsSubquery(String branch) throws NoSuchTableException {
    createAndInitUnpartitionedTable();
    ...
    sql(
        "DELETE FROM %s t WHERE NOT EXISTS (%s) OR t.id = 1",
        tableNameWithBranch(branch), subquery);
    assertEquals(
        "Should have expected rows",
        ImmutableList.of(),
        sql("SELECT * FROM %s ORDER BY id ASC NULLS LAST", tableNameWithBranch(branch)));
  ...
  protected void testDeleteWithScalarSubquery(String branch) throws NoSuchTableException {
    createAndInitUnpartitionedTable();
    ...
    withSQLConf(
        ...
        () -> {
          sql(
              "DELETE FROM %s t WHERE id <= (SELECT min(value) FROM deleted_id)",
              tableNameWithBranch(branch));
          assertEquals(
              "Should have expected rows",
              ImmutableList.of(row(2, "hardware"), row(null, "hr")),
  ...
  protected void testDeleteThatRequiresGroupingBeforeWrite(String branch)
      throws NoSuchTableException {
    createAndInitPartitionedTable();

    append(branch, new Employee(0, "hr"), new Employee(1, "hr"), new Employee(2, "hr"));
    append(branch, new Employee(0, "ops"), new Employee(1, "ops"), new Employee(2, "ops"));
    append(branch, new Employee(0, "hr"), new Employee(1, "hr"), new Employee(2, "hr"));
    append(branch, new Employee(0, "ops"), new Employee(1, "ops"), new Employee(2, "ops"));
    ...
    try {
      ...
      sql("DELETE FROM %s t WHERE id IN (SELECT * FROM deleted_id)", tableNameWithBranch(branch));
      Assert.assertEquals(
          "Should have expected num of rows", 8L, spark.table(tableNameWithBranch(branch)).count());
    } finally {
      spark.conf().set("spark.sql.shuffle.partitions", originalNumOfShufflePartitions);
    }
  }

  protected synchronized void testDeleteWithSerializableIsolation(String branch)
        ...
        executorService.submit(
            () -> {
              for (int numOperations = 0; numOperations < Integer.MAX_VALUE; numOperations++) {
                ...
                sql(
                    "DELETE FROM %s WHERE id IN (SELECT * FROM deleted_id)",
                    tableNameWithBranch(branch));
  ...
  protected synchronized void testDeleteWithSnapshotIsolation(String branch)
  ...
  protected void testDeleteRefreshesRelationCache(String branch) throws NoSuchTableException {
    createAndInitPartitionedTable();
    ...
    Dataset<Row> query =
        spark.sql("SELECT * FROM " + tableNameWithBranch(branch) + " WHERE id = 1");
    ...
    sql("DELETE FROM %s WHERE id = 1", tableNameWithBranch(branch));
  ...
  protected void testDeleteWithMultipleSpecs(String branch) {
    createAndInitTable("id INT, dep STRING, category STRING");

    // write an unpartitioned file
    append(tableNameWithBranch(branch), "{ \"id\": 1, \"dep\": \"hr\", \"category\": \"c1\"}");
    ...
    append(tableNameWithBranch(branch), "{ \"id\": 5, \"dep\": \"hr\", \"category\": \"c1\"}");

    // write another file partitioned by dep
    sql("ALTER TABLE %s DROP PARTITION FIELD category", tableName);
    append(tableNameWithBranch(branch), "{ \"id\": 7, \"dep\": \"hr\", \"category\": \"c1\"}");

    sql("DELETE FROM %s WHERE id IN (1, 3, 5, 7)", tableNameWithBranch(branch));
  ...
  protected void append(String branch, Employee... employees) throws NoSuchTableException {
    List<Employee> input = Arrays.asList(employees);
    Dataset<Row> inputDF = spark.createDataFrame(input, Employee.class);
    inputDF.coalesce(1).writeTo(tableNameWithBranch(branch)).append();
  }
