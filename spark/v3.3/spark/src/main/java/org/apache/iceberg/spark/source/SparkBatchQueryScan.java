...
class SparkBatchQueryScan extends SparkPartitioningAwareScan<PartitionScanTask>
  ...
  SparkBatchQueryScan(
    ...
    this.broadcastVarDataExpressions = Lists.newArrayList();
    this.broadcastVarPartitionExpressions = Lists.newArrayList();
  }
  ...
  public void filter(Filter[] filters) {
    Tuple<Expression, Expression> allFilterExprs = convertRuntimeFilters(filters);
    ...
    List<PartitionScanTask> filteredTasks=
        addAsDataFilters(broadcastVarPartitionExpressions, broadcastVarDataExpressions);
    ...
    filteredTasks = filterFilesAtManifestLevelAndAddToRuntimeFilters(filteredTasks,
        netPartitionFilter);
    filteredTasks = filterFilesAtDataFileLevelUsingBounds(
            netNewExprFilters, nonPartitionBasedBroadcastVar, filteredTasks);
    LOG.info(
   ...
   */
  private Tuple<Expression, Expression> convertRuntimeFilters(Filter[] filters) {
    return getExpression(filters, Expressions.alwaysTrue());
  }

  private Tuple<Expression, Expression> getExpression(Filter[] filters, Expression alWaysTrue) {
    Expression runtimeFilterExpr = alWaysTrue;
    Expression rangeInExpr = alWaysTrue;
    for (Filter filter : filters) {
      Tuple<Boolean, Expression> exprTuple = SparkFilters.convert(filter, this.expectedSchema());
