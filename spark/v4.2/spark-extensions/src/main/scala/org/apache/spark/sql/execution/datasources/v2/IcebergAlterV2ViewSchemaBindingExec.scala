    ...
    ViewUtil.icebergViewCatalog(catalog, ident) match {
      case Some(icebergViewCatalog) =>
        ...
        if (schemaMode == SchemaEvolution) {
          update.remove(SparkView.QUERY_COLUMN_NAMES)
        }
      ...
      case _ =>
        ...
        val builder =
          CatalogV2Util.viewInfoBuilderFrom(viewInfo).withSchemaMode(schemaMode.toString)
        if (schemaMode == SchemaEvolution) {
          builder.withQueryColumnNames(Array.empty[String])
        }

        CommandUtils.uncacheTableOrView(session, ResolvedIdentifier(catalog, ident))
        catalog.replaceView(ident, builder.build())
    }
