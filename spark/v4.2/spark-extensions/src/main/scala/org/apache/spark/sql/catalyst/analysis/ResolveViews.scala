...
case class ResolveViews(spark: SparkSession) extends Rule[LogicalPlan] with LookupCatalog {
  ...
  private def createViewRelation(
    ...
    // Preserve Iceberg's positional schema mapping as an alias over a projection. This differs
    // from Spark's logical View node, so CheckViews recognizes this shape when traversing V2 view
