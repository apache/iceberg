...
object CheckViews extends (LogicalPlan => Unit) {
  ...
  override def apply(plan: LogicalPlan): Unit = {
    plan foreach {
      ...
      // views into logical View nodes, which are also traversed below for cycle detection. Spark's
      // V2CreateViewPreparation still checks duplicate query column names during execution.
